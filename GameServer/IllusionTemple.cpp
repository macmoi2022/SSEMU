...
void CIllusionTemple::ProcState_EMPTY(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,729,(lpLevel->MinutesLeft+1));
			}
		}
	}
...
void CIllusionTemple::ProcState_READY(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime > 0)
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,40,(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	if(lpLevel->RemainTime <= 0)
	{
		if(lpLevel->Level == 0) { gNotice->GCNoticeSendToAll(0,0,0,0,0,0,41); }
		this->NoticeSendToAll(lpLevel,1,42,(lpLevel->Level+1),this->m_NotifyTime);
...
void CIllusionTemple::ProcState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	{
		this->NoticeSendToAll(lpLevel,0,49);
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,43);
...
void CIllusionTemple::ProcState_START(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,44);
...
void CIllusionTemple::NpcMirageTheMummy(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(IT_LEVEL_RANGE(level) == 0)
	{
		GCNewMessageSend(lpObj->Index,0,0,45);
		return;
	}
	...
	{
		GCNewMessageSend(lpObj->Index,0,0,46);
		return;
	}

	if(this->GetEnterEnabled(level) == 0)
	{
		GCNewMessageSend(lpObj->Index,0,0,50);
		return;
	}
...
void CIllusionTemple::DropEventItem(int map,int number,int aIndex) // OK
{
	...
	this->NoticeSendToAll(&this->m_IllusionTempleLevel[level],0,47,gObj[aIndex].Name);
...
void CIllusionTemple::GetEventItem(int map,int aIndex,CMapItem* lpItem) // OK
{
	...
	this->NoticeSendToAll(&this->m_IllusionTempleLevel[level],0,48,gObj[aIndex].Name);
...
void CIllusionTemple::NoticeSendToAll(ILLUSION_TEMPLE_LEVEL* lpLevel,int type,int message,...) // OK
{
	#if(GAMESERVER_UPDATE>=301 && GAMESERVER_UPDATE<=603)
...
void CIllusionTemple::CGIllusionTempleEnterRecv(PMSG_ILLUSION_TEMPLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,717,gServerInfo->m_IllusionTempleMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
