	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				info.Number = lpReadFile->GetAsNumber();

				info.StartType = lpReadFile->GetAsNumber();

				info.MonsterClass = lpReadFile->GetAsNumber();

				info.CurrentState = lpReadFile->GetAsNumber();

				info.NextIndex[1] = lpReadFile->GetAsNumber();

				info.NextIndex[2] = lpReadFile->GetAsNumber();

				info.NextIndex[3] = lpReadFile->GetAsNumber();
				...
				info.RequireMinReset = lpReadFile->GetAsNumber();

				info.RequireMaxReset = lpReadFile->GetAsNumber();

				info.RequireFamily = lpReadFile->GetAsNumber();

				info.RequireDoW = lpReadFile->GetAsNumber();

				info.RequireCDW = lpReadFile->GetAsNumber();
...
int CQuestWorld::GetStateByIndex(int QuestIndex,int QuestGroup) // OK
{
	...
	for (std::vector<QUEST_WORLD_INFO>::iterator it = this->m_QuestWorldInfo.begin(); it != this->m_QuestWorldInfo.end(); it++)
	{
		if(it->Index != QuestIndex || it->Group != QuestGroup)
		{
			continue;
		}
...
bool CQuestWorld::CharacterUseLetter(LPOBJ lpObj) // OK
{
	...
	for (std::vector<QUEST_WORLD_INFO>::iterator it = this->m_QuestWorldInfo.begin(); it != this->m_QuestWorldInfo.end(); it++)
	{
		...
		if(this->CheckQuestWorldRequisite(lpObj,&(*it)) == 0)
		{
			continue;
		}

		QUEST_WORLD_LIST* lpQuestWorldList = this->GetQuestWorldList(lpObj,it->Index,it->Group);

		if(lpQuestWorldList == 0 && it->CurrentState == QUEST_WORLD_ACCEPT)
		{
			this->AddQuestWorldList(lpObj,it->Index,it->Group,it->Number,QUEST_WORLD_ACCEPT,0);
			this->GCQuestWorldDetailSend(lpObj,it->Index,it->Group,1);
			return 1;
		}
	}
...
void CQuestWorld::CGQuestWorldListRecv(int aIndex) // OK
{
	...
	for (std::vector<QUEST_WORLD_INFO>::iterator it = this->m_QuestWorldInfo.begin(); it != this->m_QuestWorldInfo.end(); it++)
	{
		...
		if(it->MonsterClass != lpObj->QuestWorldMonsterClass)
		{
			continue;
		}
		...
		info.QuestIndex = it->Index;

		info.QuestGroup = it->Group;
...
void CQuestWorld::GCQuestWorldButtonSend(LPOBJ lpObj,int QuestIndex,int QuestGroup,int QuestOption) // OK
{
	...
	for (std::vector<QUEST_WORLD_INFO>::iterator it = this->m_QuestWorldInfo.begin(); it != this->m_QuestWorldInfo.end(); it++)
	{
		...
		if(lpObj->QuestWorldList[(it->Group-1)].QuestNumber != 0xFFFF && lpObj->QuestWorldList[(it->Group-1)].QuestNumber > it->Number)
		{
			continue;
		}

		pMsg.QuestIndex2 = it->Index;

		pMsg.QuestGroup2 = it->Group;
