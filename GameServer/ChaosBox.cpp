...
void CChaosBox::PlusItemLevelMix(LPOBJ lpObj,int type) // OK
{
	...
	{
		switch (gServerInfo->FailMixMode[lpObj->AccountLevel])
		{
		case 0:
			this->ChaosBoxInit(lpObj); // Zera o ChaosMix
			break;

		case 1:
			this->ChaosBoxItemDown(lpObj, ItemSlot); // Zera o nível do item
			break;

		case 2:
			this->ChaosBoxItemKeep(lpObj, ItemSlot); // Ignorar o ItemSlot e remover os demais
			break;
		}
		...
		gLog->Output(LOG_CHAOS_MIX, "[PlusItemLevelMix][Fail] [%s][%s] - FailMode: %d", lpObj->Account, lpObj->Name, gServerInfo->FailMixMode[lpObj->AccountLevel]);
	}
}
...
void CChaosBox::Wing2Mix(LPOBJ lpObj,int type) // OK
{
	...
	for(int n=0;n < CHAOS_BOX_SIZE;n++)
	{
		...
		else if(lpObj->ChaosBox[n].IsExcItem() != 0 && lpObj->ChaosBox[n].m_Level >= 4) //INCLUI O ITEM
		{
			ItemCount++;
			ItemMoney += lpObj->ChaosBox[n].m_BuyMoney;
		}
...
void CChaosBox::SeniorMix(LPOBJ lpObj) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,743);
		this->GCChaosMixSend(lpObj->Index,0,0);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,744);
		this->GCChaosMixSend(lpObj->Index,0,0);
		return;
	}
...
void CChaosBox::Item380Mix(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < CHAOS_BOX_SIZE;n++)
	{
		...
		if (lpObj->ChaosBox[n].Is380Item() == 0 && lpObj->ChaosBox[n].m_Level >= 4 && lpObj->ChaosBox[n].m_Option3 >= 1 && g380ItemType->Check380ItemType(lpObj->ChaosBox[n].m_Index) != 0)
		{
			ItemCount++;
			ItemSlot = n;
		}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetText(807, gServerInfo->m_ServerLang));
		this->GCChaosMixSend(lpObj->Index,7,0);
		return;
	}
