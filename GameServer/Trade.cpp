...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if (RoletaEvent.Trade(bIndex, aIndex) == false)
	{
		return;
	}
	...
	if(lpObj->Store.Open != 0 || lpTarget->Store.Open != 0)
	{
		this->GCTradeResponseSend(aIndex,0,lpTarget->Name,0,0);
		return;
	}
	...
	if (lpObj->InEvent == true || lpTarget->InEvent == true)
	{
		LogAdd(LOG_RED, "Tentei dar Trade durante o Evento");
		return;
	}
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gServerInfo->m_TradeItemBlockHarmony == 2)
	{
		if (gItemManager->CheckExchangeHarmonyItem(lpObj) == 0 || gItemManager->CheckExchangeHarmonyItem(lpTarget) == 0)
		{
			this->ResetTrade(aIndex);
			this->GCTradeResultSend(aIndex, 6);
			this->ResetTrade(bIndex);
