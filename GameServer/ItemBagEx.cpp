		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 2)
				{
					...
					this->m_BagDropType = lpReadFile->GetAsNumber();
				}
				if(section == 3)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
...
bool CItemBagEx::DropItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	{
		if (this->m_RewardType == 1 || this->m_RewardType == 2)
		{
			gObjCoinAdd(lpObj->Index, this->m_RewardCoin1, this->m_RewardCoin2, this->m_RewardCoin3);

			gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, 183, this->m_RewardCoin1, this->m_RewardCoin2, this->m_RewardCoin3);
		}

		if (this->m_RewardType == 3)
		{
			GDSetCoinSend(lpObj->Index, this->m_RewardCoin1, lpObj->Name);
	...
	for(std::map<int,ITEM_BAG_EX_INFO>::iterator it = this->m_ItemBagInfo.begin(); it != this->m_ItemBagInfo.end(); it++)
	{
		...
		if((GetLargeRand()%10000) < it->second.DropRate)
		{
			...
			for(int n=0;n < DropCount;n++)
			{
				...
				if(DropCount > 1 || it != this->m_ItemBagInfo.begin() || (gMap[map].CheckAttr(px,py,MAP_ATTR_LOCK1) != 0 || gMap[map].CheckAttr(px,py,MAP_ATTR_LOCK2) != 0))
				{
				...
				else
				{
					...
					{
						GDCreateItemSend(lpObj->Index, 0xEB, 0, 0, ItemIndex, ItemLevel, ((gItemStack->CheckItemStack(ItemIndex, ItemLevel) == 0) ? 0 : ItemGrade), ItemOption1, ItemOption2, ItemOption3, lpObj->Index, ((ItemNewOption == 0) ? ItemGrade : ItemNewOption), ItemSetOption, 0, 0, ItemSocketOption, 0xFF, ((lpItemBagItemInfo->Duration > 0) ? ((DWORD)time(0) + lpItemBagItemInfo->Duration) : 0));
