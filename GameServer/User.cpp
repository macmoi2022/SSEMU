...
void gObjCloseProc() // OK
{
	if(gCloseMsg != 0)
	{
		if((--gCloseMsgTime) <= 1)
		{
			if(gCloseMsgTime == 1)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,280);
				LogAdd(LOG_RED,gMessage->GetText(280,gServerInfo->m_ServerLang));
			}
		}
		else
		{
			...
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,281,gCloseMsgTime);
				LogAdd(LOG_RED,gMessage->GetText(281,gServerInfo->m_ServerLang),gCloseMsgTime);
			}
		}
	...
	for(int n = 0; n < MAX_OBJECT; n++)
	{
		if(gObj[n].Connected != OBJECT_OFFLINE)
		{
			if(gObj[n].Type == OBJECT_USER)
			{
				...
				if(gObj[n].Attack.Offline != 0 || gObj[n].Helper.Offline != 0 || gObj[n].Store.Offline != 0)
				{
					TotalOffline++;
				}
			}
...
void gObjAllDisconnect() // OK
{
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObj[n].Connected != OBJECT_OFFLINE && gObj[n].Type == OBJECT_USER)
		{
			...
			gCustomStore->StoreOfflineClose(&gObj[n]);
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	memset(lpObj->Store.Text,0,sizeof(lpObj->Store.Text));

	memset(lpObj->VpPShopPlayer,0,sizeof(lpObj->VpPShopPlayer));

	lpObj->Store.Open = 0;
	lpObj->Store.Transaction = 0;
	lpObj->Store.TickCount = 0;
	lpObj->Store.ItemChange = 0;
	lpObj->Store.RedrawAbs = 0;
	lpObj->Store.WantDeal = 0;
	lpObj->Store.DealerIndex = -1;
	lpObj->Store.Custom = 0;
	lpObj->Store.CustomType = 0;
	lpObj->Store.Offline = 0;
	lpObj->Store.OfflineTime = 0;
...
bool gObjGetRandomFreeLocation(int map,int* ox,int* oy,int tx,int ty,int count) // OK
{
	...
	for(int n = 0; n < count; n++)
	{
		...
		if(gMap[map].CheckAttr((*ox),(*oy),MAP_ATTR_LOCK1) == 0 && gMap[map].CheckAttr((*ox),(*oy),MAP_ATTR_LOCK2) == 0)
		{
			return 1;
		}
	}
...
bool gObjGetRandomMonsterLocation(int map,int* ox,int* oy,int tx,int ty,int count) // OK
{
	if(gMapManager->IsValidMap(map) != 0)
	{
		...
		for(int n = 0; n < count; n++)
		{
			...
			if(gMap[map].CheckAttr((*ox),(*oy),MAP_ATTR_SAFE) == 0 && gMap[map].CheckAttr((*ox),(*oy),MAP_ATTR_LOCK1) == 0 && gMap[map].CheckAttr((*ox),(*oy),MAP_ATTR_LOCK2) == 0)
			{
				return 1;
			}
		}
...
bool gObjCheckTeleportArea(int aIndex,int x,int y) // OK
{
	...
	if(gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) != 0 || gMap[lpObj->Map].CheckAttr(x,y,MAP_ATTR_SAFE) != 0)
	{
		return 0;
	}
...
bool gObjCheckMapTile(LPOBJ lpObj,int type) // OK
{
	...
	for(int x = 0; x < 3; x++)
	{
		for(int y = 0; y < 3; y++)
		{
			if(gMap[lpObj->Map].CheckAttr((lpObj->X+x),(lpObj->Y+y),MAP_ATTR_LOCK1) == 0 && gMap[lpObj->Map].CheckAttr((lpObj->X+x),(lpObj->Y+y),MAP_ATTR_LOCK2) == 0)
			{
				return 0;
			}
		}
...
void gObjTeleportMagicUse(int aIndex,int x,int y) // OK
{
	...
	if(lpObj->Attack.Offline != 0 || lpObj->Helper.Offline != 0 || lpObj->Store.Offline != 0)
	{
		return;
	}
	...
	if (lpObj->Move == true)
	{
		if (lpObj->Live == TRUE)
		{
			if (lpObj->Class == 2)
			{
				...
				gObjTeleport(lpObj->Index, 3, X, Y);
			...
			else
			{
				...
				gObjTeleport(lpObj->Index, 0, X, Y);
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	// - Events
	SobreEvent.Die(lpObj, lpTarget);
	DuelEvent.Die(lpObj);
	PairEvent.Die(lpObj);

	if(gDuel->CheckDuel(lpObj,lpTarget) == 0 && lpObj->PKLevel > 4)
	{
		if(gServerInfo->m_PKDeathAnnounce != 0)
		{
			gNotice->GCNoticeSendToAll(1,0,0,0,0,0,383,lpObj->Name,lpTarget->Name);
		}
	}
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	TheftEvent.Killer(lpObj, lpTarget);
	DeathEvent.Killer(lpObj, lpTarget);
	...
	{
		if(gCastleSiegeSync->GetState() == CS_STATE_START && lpObj->CastleJoinSide > CS_JOIN_SIDE_NONE)
		{
			return;
		}
		else if(lpObj->Y > 113)
...
int gObjMoveGate(int aIndex,int gate) // OK
{
	...
	{
		lpObj->SkillSummonPartyTime = 0;
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,272);
	}
	...
	if(lpObj->Store.Open != 0 && ((lpObj->Map == MAP_CASTLE_SIEGE && map == MAP_LORENCIA) || (lpObj->Map == MAP_LORENCIA && map == MAP_CASTLE_SIEGE)))
	{
		goto ERROR_JUMP;
	}
	...
	if(map != MAP_ACHERON1 && map != MAP_DEVENTER1)
	{
		gEffectManager->DelEffect(lpObj,EFFECT_ARCA_BATTLE_REWARD6);
	}

	if(AB_MAP_RANGE(map) == 0)
	{
		if(gArcaBattle->GetState() != AB_STATE_START)
		{
			gEffectManager->DelEffect(lpObj,EFFECT_ARCA_BATTLE_PENALTY);
		}
	}
	...
	if(TargetGate == 97)
	{
		...
		{
			if(lpObj->CastleJoinSide != CS_JOIN_SIDE_DEFENSE && gCastleSiege->CheckCastleTowerEnable() == 0)
			{
				goto ERROR_JUMP;
			}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,274);
		goto ERROR_JUMP;
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,275);
		goto ERROR_JUMP;
	}
	...
	if(map == MAP_RAKLION2)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,145);
			goto ERROR_JUMP;
		}
		else
		{
	...
	if(map == MAP_ACHERON1 || map == MAP_DEVENTER1)
	{
		gArcaBattle->GCArcaBattleOccupyZoneSend(aIndex,gate);
	}

	if(AB_MAP_RANGE(map) != 0)
	{
		if(gArcaBattle->GetState() != AB_STATE_PARTY && gArcaBattle->GetState() != AB_STATE_START && gAcheronGuardian->GetState() != AG_STATE_START && gate == 426) // Safety Acheron
		{
			goto ERROR_JUMP;
		}

		if(gArcaBattle->GetState() != AB_STATE_START && gAcheronGuardian->GetState() != AG_STATE_START && (gate == 427 || gate == 431 || gate == 439)) // Alkmar, Ubaid, Debenter
		{
			goto ERROR_JUMP;
		}
	}

	lpObj->ArcaBattleGate = gate;
...
void gObjSkillUseProc(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpObj->SkillSummonPartyTime > 0)
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,273,lpObj->SkillSummonPartyTime);
		}
	}
...
void gObjUserKill(int aIndex) // OK
{
	...
	if(lpObj->Attack.Offline != 0 || lpObj->Helper.Offline != 0 || lpObj->Store.Offline != 0)
	{
		...
		gCustomStore->StoreOfflineClose(lpObj);
...
int gObjInventorySearchSerialNumber(LPOBJ lpObj,DWORD serial) // OK
{
	...
	for(int n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,282);
...
void gObjSecondProc() // OK
{
	...
	for(int n=0;n < MAX_OBJECT;n++)
	{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				if(lpObj->ChatLimitTime > 0)
				{
					...
					if(lpObj->ChatLimitTimeSec > 60)
					{
						...
						{
							lpObj->ChatLimitTime = 0;
							gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,283);
						}
					}
				}
		...
		if(lpObj->Connected >= OBJECT_LOGGED && lpObj->Type == OBJECT_USER &&	lpObj->CloseCount > 0)
		{
			...
			{
				gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,289,(lpObj->CloseCount-1));
			}
...
bool gObjBackSpring(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 278 || lpObj->Class == 215 || lpObj->Class == 216 || lpObj->Class == 217 || lpObj->Class == 218 || lpObj->Class == 219 || gCustomMobCombo->GetMob(lpObj->Class))
	{
		return 0;
	}
	...
	if((attr & MAP_ATTR_SAFE) == MAP_ATTR_SAFE || (attr & MAP_ATTR_STAND) == MAP_ATTR_STAND || (attr & MAP_ATTR_LOCK1) == MAP_ATTR_LOCK1 || (attr & MAP_ATTR_LOCK2) == MAP_ATTR_LOCK2)
	{
		return 0;
	}

	if(lpObj->Map == MAP_CASTLE_SIEGE && (attr & MAP_ATTR_LOCK3) == MAP_ATTR_LOCK3)
	{
		return 0;
	}
...
bool BackSpringCheck(int & x,int & y,int & dir,BYTE map)
{
	...
	if ((attr & MAP_ATTR_SAFE) == MAP_ATTR_SAFE || (attr & MAP_ATTR_LOCK1) == MAP_ATTR_LOCK1 || (attr & MAP_ATTR_LOCK2) == MAP_ATTR_LOCK2 || (attr & MAP_ATTR_LOCK3) == MAP_ATTR_LOCK3)
	{
		tdir += 4;
...
void gObjCheckSelfDefense(LPOBJ lpObj,int aTargetIndex)
{
	...
	if(iPartyNumber >= 0)
	{
		for(int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			{
				gNotice->GCNoticeSend(gParty->m_PartyInfo[iPartyNumber].Index[n],1,0,0,0,0,0,284,lpObj->Name,gObj[aTargetIndex].Name);
			}
		}
	}
	else
	{
		gNotice->GCNoticeSend(aTargetIndex,1,0,0,0,0,0,284,lpObj->Name,gObj[aTargetIndex].Name);
	}

	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,284,lpObj->Name,gObj[aTargetIndex].Name);
}
...
void gObjTimeCheckSelfDefense(LPOBJ lpObj) // OK
{
	for(int n=0;n < MAX_SELF_DEFENSE;n++)
	{
		if(lpObj->SelfDefense[n] >= 0)
		{
			...
			{
				gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,285,lpObj->Name);
				gNotice->GCNoticeSend(lpObj->SelfDefense[n],1,0,0,0,0,0,285,lpObj->Name);
				lpObj->SelfDefense[n] = -1;
			}
		}
	}
...
void gObjCustomLogPlusChaosMix(LPOBJ lpObj,int type,int index) // OK
{
	...
	if (index < gServerInfo->m_AnnounceMinLevelChaosMix)
	{
		return;
	}
	...
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,500,lpObj->Name,gItemManager->GetItemName(index,(type+10)),(type+10));
	}
	else if(gServerInfo->m_ChaosMixPlusItemAnnounce == 2)
	{
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(500,gServerInfo->m_ServerLang),lpObj->Name,gItemManager->GetItemName(index,(type+10)),(type+10));
	}
}
