...
void CCustomJewelPack::CommandCustomJewelPack(LPOBJ lpObj, char* arg) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,387);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,388);
	}
}
...
void CCustomJewelPack::JewelPack(LPOBJ lpObj,char* arg,int type) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,389);
		return;
	}
	...
	if(count == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,390);
		return;
	}

	if(value > count)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,391);
		return;
	}
...
void CCustomJewelPack::JewelUnpack(LPOBJ lpObj,char* arg,int type) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,392);
		return;
	}
