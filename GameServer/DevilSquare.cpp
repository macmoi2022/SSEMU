...
void CDevilSquare::ProcState_EMPTY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,725,(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	{
		if(lpLevel->Level == 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,726);}
...
void CDevilSquare::ProcState_READY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0)
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				...
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,30,(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	if(lpLevel->RemainTime <= 0)
	{
		if(lpLevel->Level == 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,31);}
		this->NoticeSendToAll(lpLevel,1,32,(lpLevel->Level+1),this->m_NotifyTime);
...
void CDevilSquare::ProcState_STAND(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,33);
...
void CDevilSquare::ProcState_START(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,34);
...
void CDevilSquare::NpcCharon(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(DS_LEVEL_RANGE(level) == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,35);
		return;
	}
...
void CDevilSquare::NoticeSendToAll(DEVIL_SQUARE_LEVEL* lpLevel,int type,int message,...) // OK
{
	char buff[256] = {0};
...
void CDevilSquare::CGDevilSquareEnterRecv(PMSG_DEVIL_SQUARE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,715,gServerInfo->m_DevilSquareMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
