...
void CObjectManager::ObjectSetStateCreate(int aIndex) // OK
{
	...
	lpObj->Teleport = ((lpObj->Teleport==1)?((lpObj->DieRegen==0)?(((GetTickCount()-lpObj->TeleportTime)>500)?2:lpObj->Teleport):0):lpObj->Teleport);
...
void CObjectManager::ObjectMoveProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		{
			if(gMap[lpObj->Map].CheckAttr(lpObj->PathX[lpObj->PathCur],lpObj->PathY[lpObj->PathCur],MAP_ATTR_LOCK1) != 0 || gMap[lpObj->Map].CheckAttr(lpObj->PathX[lpObj->PathCur],lpObj->PathY[lpObj->PathCur],MAP_ATTR_LOCK2) != 0)
			{
				lpObj->PathCur = 0;
				lpObj->PathCount = 0;
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if(lpObj->Interface.use != 0 && (lpObj->Interface.type == INTERFACE_CHAOS_BOX || lpObj->Interface.type == INTERFACE_TRAINER))
	{
		...
		else
		{
			...
			#pragma message("Fix chaos dupe =>   Date:17/05/2025 ")
		}
	}
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	else if(lpObj->Map == MAP_CASTLE_SIEGE)
	{
		...
		{
			if(lpObj->CastleJoinSide == CS_JOIN_SIDE_DEFENSE)
			{
	...
	if (gCustomRespawn->GetRespawnLocation(lpObj) != 0) //OK
	{
		return result = gCustomRespawn->GetRespawnLocation(lpObj);
	}
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag,bool type,int AttackDamage) // OK
{
	...
	if(gMasterSkillTree->CheckMasterLevel(lpObj) == 0)
	{
		...
		experience = (experience*gHelperOffline->GetExperienceRate(lpObj))/100;
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		if(gMasterSkillTree->CheckMasterLevel(lpTarget) == 0)
		{
			...
			experience = (experience*gHelperOffline->GetExperienceRate(lpTarget))/100;
...
bool CObjectManager::CharacterLevelUp(LPOBJ lpObj,DWORD AddExperience,int MaxLevelUp,int ExperienceType) // OK
{
	if(gMasterSkillTree->CheckMasterLevel(lpObj) == 0)
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,266);
			return 1;
		}
	...
	else
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,267);
			return 1;
		}
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj,int type,int amount) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,268,gServerInfo->m_MaxStatPoint[lpObj->AccountLevel]);
		return 0;
	}
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	#endif
	else if (TempInventory[INVENTORY_SLOT_WING] >= 36 && TempInventory[INVENTORY_SLOT_WING] <= 40 || gCustomWing->CheckCustomWingByItem(GET_ITEM(12, TempInventory[INVENTORY_SLOT_WING])) != 0)
	{
		lpObj->CharSet[5] |= (((3) & 0x03) << 2);
		lpObj->CharSet[17] |= (TempInventory[INVENTORY_SLOT_WING] - 35 + 1) << 2;
	}
	#endif
	...
	if (gCustomPet->CheckCustomPetByItem(GET_ITEM(13, TempInventory[INVENTORY_SLOT_HELPER])) != 0)
	{
		if (TempInventory[INVENTORY_SLOT_HELPER] > 69)
		{
			lpObj->CharSet[5] |= 3;
			lpObj->CharSet[16] |= (TempInventory[INVENTORY_SLOT_HELPER] - GET_ITEM(13, 69) + 1) << 1;
		}
	}
}
...
{
	if (gServerInfo->m_CheckAutoPotionHack != 0 && gServerInfo->m_CheckAutoPotionHackAction != 0)
	{
	...
	{
		if (GetTickCount() < lpObj->PotionTime)
		{
			return 0;
		}
	}
	...
	if (gServerInfo->m_CheckAutoPotionHack != 0)
	{
		lpObj->PotionTime = GetTickCount() + gServerInfo->m_CheckAutoPotionHackTolerance;
	}
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if((GetLargeRand()%100) < gCustomJewel->GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index,lpObj->AccountLevel))
	{
		...
		#if(GAMESERVER_UPDATE>=601)
		lpItem->m_Level = (((lpItem->m_Level+lpInfo->Level)> gServerInfo->m_MaxItemLevel)? gServerInfo->m_MaxItemLevel :(lpItem->m_Level+lpInfo->Level));
		#else
		lpItem->m_Level = (((lpItem->m_Level+lpInfo->Level)> gServerInfo->m_MaxItemLevel)? gServerInfo->m_MaxItemLevel :(lpItem->m_Level+lpInfo->Level));
		#endif
...
void CObjectManager::CharacterAutoRecuperation(LPOBJ lpObj) // OK
{
	...
	if(((lpObj->SDRecoveryCount++)%5) == 0)
	{
		...
		{
			if(lpObj->SDRecoveryType != 0 || gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) != 0)
			{
		...
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			switch(n)
			{
				...
				case 6:
					...
					break;
				case 7:
					result = lpObj->Inventory[n].WingDurabilityDown(lpObj->Index,((gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE)==0)?1:0));
					break;
				case 9:
					result = lpObj->Inventory[n].PendantDurabilityDown(lpObj->Index,((gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE)==0)?1:0));
					break;
			}
		...
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			switch(lpObj->Inventory[n].m_Index)
			{
				case GET_ITEM(13,10):
					...
					break;
				case GET_ITEM(13,20):
					result = lpObj->Inventory[n].RingDurabilityDown(lpObj->Index,((gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE)==0)?70:0));
					break;
				case GET_ITEM(13,38):
					result = lpObj->Inventory[n].RingDurabilityDown(lpObj->Index,((gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE)==0)?63:0));
					break;
				case GET_ITEM(13,39):
					result = lpObj->Inventory[n].RingDurabilityDown(lpObj->Index,113);
					break;
				...
				case GET_ITEM(13,122):
					...
					break;
				default:
					result = lpObj->Inventory[n].RingDurabilityDown(lpObj->Index,((gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE)==0)?1:0));
					break;
			}
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	if(lpObj->Class == CLASS_DW)
	{
		...
		{
			gServerInfo->m_CalcCharacter.Execute("DWElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	if(lpObj->Class == CLASS_DK)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("DKElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_FE)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("FEElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_MG)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("MGElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_DL)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("DLElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_SU)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("SUElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_RF)
	{
		if(gServerInfo->m_CalcLuaScriptSwitch != 0)
		{
			gServerInfo->m_CalcCharacter.Execute("RFElementalCalculate","i>iiiii",aIndex,&lpObj->ElementalDefense,&lpObj->ElementalDamageMin,&lpObj->ElementalDamageMax,&lpObj->ElementalAttackSuccessRate,&lpObj->ElementalDefenseSuccessRate);
		}
		else
		{
	...
	for (int i = 0; i < 12; i++)
	{
		...
		{
			gItemAttribute->SetValues(lpObj, lpObj->Inventory[i].m_Index);
		}
	}
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	else if(lpObj->Level < 6 && gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) == 0)
	{
	...
	if(gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_LOCK1) != 0 || gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_LOCK2) != 0)
	{
		gMap[lpObj->Map].GetMapRandomPos(&lpObj->X,&lpObj->Y,18);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage,bool combo) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if(AB_MAP_RANGE(lpTarget->Map) != 0)
			{
				gArcaBattle->MonsterDieProc(lpTarget,&gObj[SummonIndex]);
			}
	...
	else
	{
		...
		{
			GCDamageSend(SummonIndex,lpTarget->Index,flag,damage,DAMAGE_TYPE_POISON,ShieldDamage);
		}
		else if(DamageType == 3)
		{
			GCDamageSend(SummonIndex,lpTarget->Index,flag,damage,DAMAGE_TYPE_DEBUFF,ShieldDamage);
		}
		#if(GAMESERVER_UPDATE>=701)
...
bool CObjectManager::CharacterUseJewelOfEvolution(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		return 0;
	...
	for (std::map<int, CUSTOMEVO_INFO>::iterator it = gCustomEvo->m_CustomEvo.begin(); it != gCustomEvo->m_CustomEvo.end(); it++)
	{
		if (lpItem->m_Index == it->second.ItemIndex)
		{
			lpItem->m_Index = it->second.ResultIndex;
			return 1;
		}
	}
...
bool CObjectManager::CharacterUseJewelOfSetEvolution(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		return 0;
	...
	for (std::map<int, CUSTOMEVO_INFO>::iterator it = gCustomEvo->m_CustomEvo.begin(); it != gCustomEvo->m_CustomEvo.end(); it++)
	{
		if (lpItem->m_Index == it->second.ItemIndex)
		{
			if (gItemManager->CheckItemInventorySpace(lpObj, it->second.ResultIndex) == 0)
			{
				gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You do not have space  to use the Item!");
				GCLifeSend(lpObj->Index, 0xFD, (BYTE)lpObj->Life, lpObj->Shield);
				return 0;
			}

			lpItem->m_Index = it->second.ResultIndex;

			GDCreateItemSend(lpObj->Index, 0xEB, 0, 0, lpItem->m_Index, lpItem->m_Level, (BYTE)lpItem->m_Durability, lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3, lpObj->Index, lpItem->m_NewOption, lpItem->m_SetOption, lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx, lpItem->m_SocketOption, lpItem->m_SocketOptionBonus, 0);
