...
void RenderMenu(HWND hWnd) // OK
{
	...
	AppendMenu(hEvents,MF_STRING,IDM_EVENT_FORCEQUIZ, "Start CustomQuiz");
	...
	AppendMenu(hReload,MF_STRING,IDM_RELOAD_RELOADLANG,"Reload Lang");
	...
	AppendMenu(hHelp,MF_STRING,IDM_WZTEAM,"WZTEAM Web");
		...
		case WM_COMMAND:
			switch(LOWORD(wParam))
			{
				case IDM_EXIT:
					if(gObjTotalUser != 0)
					{
						MessageBox(hWnd,"Cannot close gameserver with connected players,please disconnect them first!","Danger of data loss!",MB_OK | MB_ICONEXCLAMATION);
						break;
					}
				...
				case IDM_FILE_1MINUTESERVERCLOSE:
					if(gCloseMsg == 0)
					{
						gCloseMsg = 1;
						gCloseMsgTime = 60;
						gNotice->GCNoticeSendToAll(0,0,0,0,0,0,286,gCloseMsgTime/60);
					}
					break;
				...
				case IDM_RELOAD_RELOADITEM:
					...
					break;
				case IDM_RELOAD_RELOADLANG:
					gServerInfo->ReadLangInfo();
					LogAdd(LOG_BLUE,"[ServerInfo] Lang reloaded successfully");
					break;
				...
				case IDM_ABOUT:
					...
					break;
				case IDM_WZTEAM:
					ShellExecute(0,0,"https://www.wizardteam.com.br",0,0,SW_SHOW);
					break;
				default:
	...
	else if(message == WM_TIMER && wParam == MAP_VIEWER_TIMER)
	{
		...
		if(gMap[map].m_MapAttr != 0)
		{
			for (int x = 0; x < 255; x++)
			{
				for (int y = 0; y < 255; y++)
				{
					if(gMap[map].CheckAttr(x,y,MAP_ATTR_LOCK1) != 0 || gMap[map].CheckAttr(x,y,MAP_ATTR_LOCK2) != 0)
					{
						rect.top = margim+x*3;
		...
		for (int n = 0; n < MAX_OBJECT; n++)
		{
			...
			if(lpTarget->Type == OBJECT_USER && MapShowPlayer != 0)
			{
				...
				if(lpTarget->Helper.Offline != 0 || lpTarget->Attack.Offline != 0 || lpTarget->Store.Offline != 0)
				{
