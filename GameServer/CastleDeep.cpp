...
void CCastleDeep::ProcState_EMPTY() // OK
{
	if(this->m_RemainTime > 0 && this->m_RemainTime <= 300 && this->m_TimeNotify == 0)
	{
		...
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(52,gServerInfo->m_ServerLang));
	}

	if(this->m_RemainTime <= 0)
	{
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(53,gServerInfo->m_ServerLang));
...
void CCastleDeep::ProcState_START() // OK
{
	...
	if(this->m_RemainTime <= 0)
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,54);
...
void CCastleDeep::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,55,lpObj->Name,lpTarget->Name);
	}
}
