...
bool CDarkSpirit::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,BYTE action) // OK
{
	...
	if(AB_MAP_RANGE(lpObj->Map) || AB_MAP_RANGE(lpTarget->Map))
	{
		if(gArcaBattle->GetState() == AB_STATE_START)
		{
			...
			{
				if(lpTarget->Class == OBJECT_USER && gArcaBattle->CheckPlayerTarget(lpObj,lpTarget) == 0)
				{
					return 0;
				}

				if(lpTarget->Class == OBJECT_MONSTER && gArcaBattle->CheckMonsterTarget(lpObj,lpTarget) == 0)
				{
					return 0;
				}
			}
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiegeSync->GetState() == CS_STATE_START && lpObj->CastleJoinSide > CS_JOIN_SIDE_NONE)
			{
				CheckSelfDefense = 0;
			}
