		...
		while (true)
		{
			...
			while (true)
			{
				...
				else if (section == 3)
				{
					...
					info.CountMessage = lpReadFile->GetAsNumber();
				...
				for (int n = 0;n < it->Count;n++)
				{
					...
					for (int m = 0; m < MAX_INVASION_RESPAWN_GROUP; m++)
					{
						if (lpInfo->InfoMonter[m].Class == lpObj->Class && lpInfo->InfoMonter[m].Class > 0)
						{
							...
							lpInfo->InfoMonter[m].RegenType = ((it->RegenType == 0) ? (lpInfo->InvasionTime * 1000) : it->RegenTime);
					...
					for (int i = 0; i < MAX_INVASION_RESPAWN_GROUP; i++)
					{
						if (!check && lpInfo->InfoMonter[i].Class == -1)
						{
							...
							lpInfo->InfoMonter[i].RegenType = ((it->RegenType == 0) ? (lpInfo->InvasionTime * 1000) : it->RegenTime);
...
void CInvasionManager::MonsterDieProc(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	for (int m = 0; m < MAX_INVASION_RESPAWN_GROUP; m++)
	{
		...
		if (lpInfo->InfoMonter[m].Class == lpObj->Class)
		{
			...
			if (lpInfo->MonsterInfo[m].CountMessage == 1)
			{
				if (lpInfo->InfoMonter[m].Count == 0)
				{
					gNotice->GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage->GetMessage(1100), gMonsterManager->GetMonsterName(lpInfo->InfoMonter[m].Class));
				}
				else
				{
					gNotice->GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage->GetMessage(1101), lpTarget->Name, gMonsterManager->GetMonsterName(lpInfo->InfoMonter[m].Class), (lpInfo->InfoMonter[m].CountMax - lpInfo->InfoMonter[m].Count), lpInfo->InfoMonter[m].CountMax);
				}
			}
