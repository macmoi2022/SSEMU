...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg) // OK
{
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		{
			info.CharSet[5] |= (((3) & 0x03) << 2);
			info.CharSet[17] |= (TempInventory[INVENTORY_SLOT_WING] - 35 + 1) << 2;
		}
		...
		if (gCustomPet->CheckCustomPetByItem(GET_ITEM(13, TempInventory[INVENTORY_SLOT_HELPER])) != 0)
		{
			...
			{
				info.CharSet[5] |= 3;
				info.CharSet[16] |= ((TempInventory[INVENTORY_SLOT_HELPER] - 69) + 1) << 1;
			}
		}
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	gLuckyWheel->GCLuckyWheelSend(lpObj->Index);
	gLuckyWheel->SendInfo(lpObj->Index);
	...
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(800, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 3;
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(802, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 3;
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(804, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 3;
		return;
	}

	if (gServerInfo->m_ServerMaxLevel != 0 && lpObj->AccountLevel > gServerInfo->m_ServerMaxLevel)
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(801, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 3;
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(803, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 4;
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpMsg->index, 0, 0, 0, 0, 0, 0, gMessage->GetText(805, gServerInfo->m_ServerLang));
		lpObj->CloseCount = 6;
		lpObj->CloseType = 4;
		return;
	}
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	{
		case 0:
			PostMessage1(lpMsg->name,lpMsg->code,lpMsg->message);
			break;
		case 1:
			PostMessage2(lpMsg->name,lpMsg->code,lpMsg->message);
			break;
		case 2:
			PostMessage3(lpMsg->name,lpMsg->code,lpMsg->message);
			break;
		case 3:
			PostMessage4(lpMsg->name,lpMsg->code,lpMsg->message);
			break;
		case 4:
...
void DGGlobalMessageRecv(SDHP_GLOBAL_MESSAGE_RECV* lpMsg) // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		{
			GCNewMessageSend(n,lpMsg->type,lpMsg->color,"%s",lpMsg->message);
		}
	}
}
...
void GDRankingUpdateSend(int aIndex, char* Coluna ,DWORD Score) // OK
{
	LPOBJ lpObj = &gObj[aIndex];

	PMSG_HERO_COUNT_SEND pMsg;
	...
	memcpy(pMsg.coluna, Coluna, sizeof(pMsg.coluna));

	pMsg.score = Score;

	LogAdd(LOG_GREEN,"[ RANKING ] Name : %s - Rank %s - Score %d",lpObj->Name, Coluna, Score);

	gDataServerConnection.DataSend((BYTE*)&pMsg, pMsg.header.size);
}

void  GDAtualizarPatenteSend(int aIndex, int patente) // OK
{
	LPOBJ lpObj = &gObj[aIndex];

	lpObj->Patente = patente;

	SDHP_UPDATE_PATENTE_SEND pMsg = { 0 };

	pMsg.header.set(0xF3, 0xE7, sizeof(pMsg));

	memcpy(pMsg.nome, lpObj->Name, sizeof(pMsg.nome)); // Envia vencedor

	for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
	{
		LPOBJ lpUser = &gObj[i];

		if (!lpUser->Connected || lpUser->Type != OBJECT_USER)
			continue;

		if (lpUser->Patente == patente)
		{
			lpUser->Patente = 0; // remove da memória

		}
	}

	pMsg.patente = patente; // Envia patente
...
void GDSetCoinSend(int aIndex, int Gold, char* LogName) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	pMsg.Gold = (!Gold) ? 0 : Gold;

	gDataServerConnection.DataSend((BYTE*)&pMsg, pMsg.header.size);

	LogAdd(LOG_GREEN, "[%s][%s] Reward(%s) Gold: %d", lpObj->Account, lpObj->Name, LogName, Gold);
}