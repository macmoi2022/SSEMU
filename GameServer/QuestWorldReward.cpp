	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				info.Number = lpReadFile->GetNumber();
				...
				info.ItemSortRate = lpReadFile->GetAsNumber();
...
void CQuestWorldReward::GCQuestWorldDetailSend(LPOBJ lpObj,int QuestIndex,int QuestGroup,BYTE protocol) // OK
{
	...
	for (std::vector<QUEST_WORLD_REWARD_INFO>::iterator it = this->m_QuestWorldRewardInfo.begin(); it != this->m_QuestWorldRewardInfo.end(); it++)
	{
		...
		if(this->CheckQuestWorldRewardRequisite(lpObj,&(*it)) == 0)
		{
			continue;
		}

		pMsg.QuestReward[pMsg.RewardCount].type = it->Type;

		pMsg.QuestReward[pMsg.RewardCount].index = it->Index;

		pMsg.QuestReward[pMsg.RewardCount].value = it->Value;

		if(it->Type == QUEST_WORLD_REWARD_ITEM)
		{
			CItem item;
