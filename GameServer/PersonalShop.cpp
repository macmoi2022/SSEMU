...
void CPersonalShop::CGPShopOpenRecv(PMSG_PSHOP_OPEN_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		lpObj->Store.Open = 1;
		memcpy(lpObj->Store.Text,lpMsg->text,sizeof(lpMsg->text));
	}
	else
	{
		memcpy(lpObj->Store.Text,lpMsg->text,sizeof(lpMsg->text));
	...
	if(gCustomStore->m_CustomStoreSwitch != 0)
	{
		if(lpMsg->text[0] == '/')
		{
			if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoBSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,0);
			}
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoSSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,1);
			}
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoLSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,2);
			}
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoCSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,3);
			}
			if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJCRSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,4);
			}
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoGSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,5);
			}
			#if(GAMESERVER_UPDATE>=201)
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandJoHSyntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,6);
			}
			#endif
			#if(GAMESERVER_SHOP>=1)
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandCoin1Syntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,7);
			}
			#endif
			#if(GAMESERVER_SHOP==3)
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandCoin2Syntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,8);
			}
			else if(strcmp(&lpMsg->text[1],gCustomStore->m_CustomStoreCommandCoin3Syntax) == 0)
			{
				gCustomStore->OpenCustomStore(lpObj,9);
			}
			#endif
		}
	}
...
void CPersonalShop::CGPShopItemListRecv(PMSG_PSHOP_ITEM_LIST_RECV* lpMsg,int aIndex) // OK
{
	...
	lpObj->Store.WantDeal = 1;
	lpObj->Store.DealerIndex = bIndex;
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if((GetTickCount()-lpObj->Store.TickCount) < 2000)
	{
		return;
	}

	lpObj->Store.TickCount = GetTickCount();
...
void CPersonalShop::CGPShopSearchRecv(PMSG_PSHOP_SEARCH_RECV* lpMsg,int aIndex) // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		if((pMsg.count++) >= lpMsg->count)
		{
			...
			memcpy(info.text,lpTarget->Store.Text,sizeof(lpTarget->Store.Text));
...
void CPersonalShop::GCPShopViewportSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpObj->VpPShopPlayerCount == 0)
		{
			...
			memcpy(info.text,lpTarget->Store.Text,sizeof(info.text));
	...
	if(lpObj->Store.WantDeal != 0)
	{
		if(gObjIsConnectedGP(lpObj->Store.DealerIndex) == 0 || gObj[lpObj->Store.DealerIndex].Store.Open == 0)
		{
			lpObj->Store.WantDeal = 0;
			lpObj->Store.DealerIndex = -1;
			this->GCPShopLeaveSend(aIndex,lpObj->Store.DealerIndex);
		}
		else if(gObj[lpObj->Store.DealerIndex].Store.ItemChange != 0)
		{
			this->GCPShopItemListSend(aIndex,lpObj->Store.DealerIndex,1,1);
		}
	}
}
...
void CPersonalShop::GCPShopTextChangeSend(int aIndex) // OK
{
	...
	memcpy(pMsg.text,gObj[aIndex].Store.Text,sizeof(pMsg.text));
