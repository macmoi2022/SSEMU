...
void CGuild::DGGuildAssignStatusRecv(SDHP_GUILD_ASSIGN_STATUS_RECV* lpMsg) // OK
{
	...
	if(lpGuild != 0)
	{
		...
		if(lpMsg->result == 1)
		{
			...
			{
				this->NoticeSendToAll(lpGuild,0,190,lpMsg->Member);
			}
			else if(lpMsg->Status == GUILD_STATUS_BATTLE_MASTER)
			{
				this->NoticeSendToAll(lpGuild,0,191,lpMsg->Member);
			}
			else
			{
				this->NoticeSendToAll(lpGuild,0,192,lpMsg->Member);
			}
		}
	}
...
void CGuild::DGGuildAssignTypeRecv(SDHP_GUILD_ASSIGN_TYPE_RECV* lpMsg) // OK
{
	...
	if(lpGuild != 0)
	{
		...
		if(lpMsg->result == 1)
		{
			...
			this->NoticeSendToAll(lpGuild,0,193,lpMsg->Name,lpMsg->Type);
...
void CGuild::DGUnionClearRecv(SDHP_UNION_CLEAR_RECV* lpMsg) // OK
{
	...
	if(lpGuild != 0)
	{
		...
		this->NoticeSendToAll(lpGuild,1,201,lpGuild->Name);
	}
}
...
void CGuild::DGGuildUnionJoinRecv(SDHP_GUILD_UNION_JOIN_RECV* lpMsg) // OK
{
	...
	if(lpMsg->relation == GUILD_RELATION_UNION)
	{
		this->NoticeSendToAll(lpGuildA,1,194,lpGuildB->Name);

		this->NoticeSendToAll(lpGuildB,1,196,lpGuildA->Name,lpGuildB->Name);
	}
	else if(lpMsg->relation == GUILD_RELATION_RIVAL)
	{
		this->NoticeSendToAll(lpGuildA,1,195,lpGuildB->Name);
		
		this->NoticeSendToAll(lpGuildB,1,195,lpGuildA->Name);
	}
}
...
void CGuild::DGGuildUnionBreakRecv(SDHP_GUILD_UNION_BREAK_RECV* lpMsg) // OK
{
	...
	if (lpMsg->relation == GUILD_RELATION_UNION)
	{
		this->NoticeSendToAll(lpGuildA,1,197);
	}
	else if(lpMsg->relation == GUILD_RELATION_RIVAL)
	{
		this->NoticeSendToAll(lpGuildA,1,200,lpGuildB->Name);

		this->NoticeSendToAll(lpGuildB,1,200,lpGuildA->Name);
	}
}
...
void CGuild::DGGuildAllianceKickRecv(SDHP_GUILD_ALLIANCE_KICK_RECV* lpMsg) // OK
{
	...
	this->NoticeSendToAll(lpGuildA,1,198,lpMsg->GuildNameA,lpMsg->GuildNameB);

	this->NoticeSendToAll(lpGuildB,1,197);
}
...
void CGuild::GDGuildScoreUpdateSend(char* GuildName,int score) // OK
{
	...
	#pragma message("Bug Fix => Guild Name NULL  Date:17/05/2025 ")

	if (GuildName[0] == '\0' || strlen(GuildName) < 4 || GuildName == NULL)
	{
		return;
	}
...
void CGuild::CGGuildRequestRecv(PMSG_GUILD_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,740);
		return;
	}
...
void CGuild::CGGuildResultRecv(PMSG_GUILD_RESULT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gCastleSiegeSync->GetState() == CS_STATE_START)
	{
		gNotice->GCNoticeSend(bIndex,1,0,0,0,0,0,740);
		goto CLEAR_JUMP;
	}
...
BOOL Protect(LPOBJ lpObj)
{
	if ((GetTickCount() - lpObj->m_Packet) < 300)
	{
		return FALSE;
	}

	lpObj->m_Packet = GetTickCount();
	return TRUE;
}
...
void CGuild::CGGuildDeleteRecv(PMSG_GUILD_DELETE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->Lock > 0) 
	{
		...
		DataSend(aIndex, (BYTE*)&lpMsg, lpMsg->header.size);
		return;
	}

	if(gCastleSiegeSync->GetState() == CS_STATE_START)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,741);
		return;
	}
			...
			if(lpGuild2 != 0)
			{
				if(lpGuild2->WarDeclare == GUILD_WAR_STATE_DECLARE)
				{
					...
					if(lpGuild1->WarType == GUILD_WAR_TYPE_SOCCER)
					{
						...
						this->NoticeSendToAll(lpGuild1,1,287);

						this->NoticeSendToAll(lpGuild2,1,287);
					}
				}
			}
		}
...
void CGuild::CGGuildUnionRequestRecv(PMSG_GUILD_UNION_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,742);
...
void CGuild::GuildMasterUpdate(char* Name) // OK
{
...
void CGuild::NoticeSendToAll(GUILD_INFO* lpGuild,int type,int message,...) // OK
{
	if(lpGuild != 0)
	{
		char buff[256] = {0};
		...
		for(int n=0;n < MAX_GUILD_MEMBER;n++)
		{
			if(lpGuild->Member[n].IsValid() != 0)
			{
				...
				gNotice->GCNoticeSend(lpGuild->Member[n].Index,type,0,0,0,0,0,"%s",buff);
			}
		}
	}
}
