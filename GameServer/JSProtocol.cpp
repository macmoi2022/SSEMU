...
void JGConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg) // OK
{
	...
	{
		GJDisconnectAccountSend(lpMsg->index,lpMsg->account,gObj[lpMsg->index].IpAddr,gObj[lpMsg->index].HardwareId);
		GCConnectAccountSend(lpMsg->index,10);
		return;
	}
...
void JGMapServerMoveRecv(SDHP_MAP_SERVER_MOVE_RECV* lpMsg) // OK
{
	...
	if(lpMsg->result == 0)
	{
		...
		gNotice->GCNoticeSend(lpMsg->index,0,0,0,0,0,0,259);
		return;
	}
...
void JGMapServerMoveAuthRecv(SDHP_MAP_SERVER_MOVE_AUTH_RECV* lpMsg) // OK
{
	...
	{
		GJDisconnectAccountSend(lpMsg->index,lpMsg->account,gObj[lpMsg->index].IpAddr,gObj[lpMsg->index].HardwareId);
		GCMapServerMoveAuthSend(lpMsg->index,2);
		gLog->Output(LOG_CONNECT,"[ObjectManager][%d] CloseClient [%s] Reason: MoveAuth ConnectMemberCheck [%d]",lpMsg->index,gObj[lpMsg->index].IpAddr,gServerInfo->m_ConnectMemberCheck);
		CloseClient(lpMsg->index);
		return;
	}
	...
	if(lpMsg->BlockCode != 0)
	{
		...
		gLog->Output(LOG_CONNECT,"[ObjectManager][%d] CloseClient [%s] Reason: MoveAuth BlockCode [%d]",lpMsg->index,gObj[lpMsg->index].IpAddr,lpMsg->BlockCode);
		CloseClient(lpMsg->index);
		return;
	}
	...
	if(gServerInfo->m_ServerLock > lpMsg->AccountLevel)
	{
		...
		gLog->Output(LOG_CONNECT,"[ObjectManager][%d] CloseClient [%s] Reason: MoveAuth ServerLock [%d][%d]",lpMsg->index,gObj[lpMsg->index].IpAddr,lpMsg->AccountLevel,gServerInfo->m_ServerLock);
		CloseClient(lpMsg->index);
		return;
	}
	...
	if(AB_MAP_RANGE(lpMsg->map) != 0)
	{
		if(gArcaBattle->GetState() == AB_STATE_PARTY || gArcaBattle->GetState() == AB_STATE_START)
		{
			gArcaBattle->DeleteBuffGuildWinner(lpMsg->index);

			ARCA_BATTLE_USER_INFO* lpUser = gArcaBattle->GetUser(lpMsg->name);

			if(lpUser != 0)
			{
				lpUser->Index = lpMsg->index;

				gArcaBattle->GCArcaBattlePlayInfoSend(lpMsg->index);

				gArcaBattle->GCArcaBattleStateSend(gArcaBattle->GetState()+1,lpMsg->index);

				gArcaBattle->GCArcaBattlePlayerPositionSend();
			}
		}
		else
		{
...
void JGAccountLevelRecv(SDHP_ACCOUNT_LEVEL_RECV* lpMsg) // OK
{
	...
	else
	{
		gObj[lpMsg->index].AccountLevel = lpMsg->AccountLevel;
		memcpy(gObj[lpMsg->index].AccountExpireDate,lpMsg->AccountExpireDate,sizeof(gObj[lpMsg->index].AccountExpireDate));
		gNotice->GCNoticeSend(lpMsg->index,1,0,0,0,0,0,(155+gObj[lpMsg->index].AccountLevel),gObj[lpMsg->index].AccountExpireDate);
	}
}
...
void JGAccountAlreadyConnectedRecv(SDHP_ACCOUNT_ALREADY_CONNECTED_RECV* lpMsg) // OK
{
	...
	gCustomStore->StoreOfflineClose(&gObj[lpMsg->index]);
