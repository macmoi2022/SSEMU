...
bool CItemManager::CheckItemMoveToBlock(LPOBJ lpObj,CItem* lpItem) // OK
{
	...
	if(gServerInfo->m_TradeItemBlockHarmony == 1 && lpItem->IsJewelOfHarmonyItem() != 0)
	{
		return 0;
	}
...
bool CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpItem->m_Index == GET_ITEM(14, 15)) // Dinheiro
	{
		bool isPartyActive = (OBJECT_RANGE(lpObj->PartyNumber) != 0 && gServerInfo->m_PartyMoneyDistribute != 0);

		int totalMoney = lpItem->m_BuyMoney;

		// Registra o item no mapa
		gMap[lpObj->Map].ItemGive(aIndex, index);

		if (isPartyActive) // Distribuição em grupo
		{
			int partyMemberCount = 0;

			// Primeiro, conte apenas os membros dentro do alcance
			for (int n = 0; n < MAX_PARTY_USER; n++)
			{
				int bIndex = gParty->m_PartyInfo[lpObj->PartyNumber].Index[n];

				if (OBJECT_RANGE(bIndex) == 0) continue;

				LPOBJ lpTarget = &gObj[bIndex];

				if (lpObj->Map == lpTarget->Map && gObjCalcDistance(lpObj, lpTarget) < MAX_PARTY_DISTANCE)
				{
					partyMemberCount++; // Conta apenas os membros dentro da distância
				}
			}

			if (partyMemberCount > 0) // Apenas distribua se houver membros no alcance
			{
				for (int n = 0; n < MAX_PARTY_USER; n++)
				{
					int bIndex = gParty->m_PartyInfo[lpObj->PartyNumber].Index[n];
					...
					if (lpObj->Map == lpTarget->Map && gObjCalcDistance(lpObj, lpTarget) < MAX_PARTY_DISTANCE)
					{
						int partyMoney = totalMoney / partyMemberCount;

						if (gObjCheckMaxMoney(bIndex, partyMoney) == 0)
						{
							lpTarget->Money = MAX_MONEY;
						}
						else
						{
							lpTarget->Money += partyMoney;
						}
		...
		{
			if (gObjCheckMaxMoney(aIndex, totalMoney) == 0)
			{
				lpObj->Money = MAX_MONEY;
			}
			else
			{
				lpObj->Money += totalMoney;
			}
	...
	if(this->InventoryInsertItemStack(lpObj,&item) != 0)
	{
		...
		{
			gArcaBattle->GetEventItem(aIndex,lpItem);
		}
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gCustomPet->CheckCustomPetByItem(lpItem->m_Index) != 0)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
	...
	if(lpItem->m_Index == GET_ITEM(13,20) && lpItem->m_Level != 0) // Starter Ring 40 && Starter Ring 80
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,257);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,258);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		if(lpObj->Store.Open != 0 || lpObj->Store.Transaction != 0)
		{
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex) // OK
{
	...
	else if (lpItem->m_Index == GET_ITEM(13, 60) && lpObj->PKLevel > 3) // Indulgence
	{
		gObjPKDownCheckTime(lpObj, gServerInfo->m_IndulgenceTimeConstA, 0);
		this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
		this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
	}
	...
	else if ((lpItem->m_Index >= GET_ITEM(14, 0) && lpItem->m_Index <= GET_ITEM(14, 6)) || (lpItem->m_Index >= GET_ITEM(14, 35) && lpItem->m_Index <= GET_ITEM(14, 40)) || lpItem->m_Index == GET_ITEM(14, 70) || lpItem->m_Index == GET_ITEM(14, 71) || lpItem->m_Index == GET_ITEM(14, 133))
	{
		...
		{
			if (gServerInfo->m_InfinityPotionRate[lpObj->AccountLevel] == 0)
			{
				this->DecreaseItemDur(lpObj, lpMsg->SourceSlot, 1);
			}			
		}
	...
	else if (lpItem->m_Index == GET_ITEM(14, 12)) // sistema WZen
	{
		if (lpObj->Money > gServerInfo->m_eZenReward)
		{
			gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetMessage(904));
			return;
		}
		else
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
			GCMoneySend(lpObj->Index, lpObj->Money += gServerInfo->m_eZenReward);
			gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetMessage(905));
		}

	}
	else if (lpItem->m_Index == GET_ITEM(14, 207))
	{

		int AccountLevel = gServerInfo->AccountLevelBox[0];
		int Days = gServerInfo->DaysBox[0];

		if (lpObj->AccountLevel > AccountLevel)
		{
		...
		GJAccountLevelSaveSend(lpObj->Index, AccountLevel, (Days * 86400));
		GJAccountLevelSend(lpObj->Index);

		if (lpObj->Index != lpObj->Index)
		{
			gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetMessage(377), lpObj->AccountExpireDate);
		}

		gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetMessage(378));

		this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
		this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
	}
	else if (lpItem->m_Index == GET_ITEM(14, 208))
	{

		int AccountLevel = gServerInfo->AccountLevelBox[1];
		int Days = gServerInfo->DaysBox[1];
	...
	else if (lpItem->m_Index == GET_ITEM(14, 209))
	{
		...
		int AccountLevel = gServerInfo->AccountLevelBox[2];
		int Days = gServerInfo->DaysBox[2];
	...
	else if (lpItem->m_Index == GET_ITEM(14, 211))
	{
		if (gObjectManager->CharacterUseJewelOfSetEvolution(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot) != 0)
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
			this->GCItemModifySend(aIndex, lpMsg->TargetSlot);
	...
	else if (lpItem->m_Index == GET_ITEM(14, 212))
	{
		int Effect = 30;
		int Type = 2;
		int Duration = gServerInfo->m_SealEffectTime;
		int Power1 = 0;
		int Power2 = 0;
		int Power3 = 0;
		int Power4 = 0;
		...
		if (Effect != -1)
		{
			...
			{
				gEffectManager->AddEffect(lpObj, (Type > 0) ? 1 : 0, Effect, (Type == 0) ? Duration : (int)(time(0) + Duration), Power1, Power2, Power3, Power4);
			}
		}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(type == 1)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,384,item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	else if(type == 2)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,385,item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	else if(type == 3)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,386,item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gServerInfo->SellItemExc != 0)
	{
		if (lpItem->m_NewOption >= 16 && lpItem->m_Level >= 10)
		{
			gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Dont Sell Item Reforced");
...
void CItemManager::DGGetItemSerialRecv(SDHP_GET_ITEM_SERIAL_RECV* lpMsg) // OK
{
	...
	if(AB_MAP_RANGE(lpObj->Map) != 0)
	{
		gArcaBattle->GetEventItem(lpObj->Index,lpItem);
	}
...
bool CItemManager::CheckExchangeHarmonyItem(LPOBJ lpObj) // OK
{
	for (int n = 0; n < TRADE_SIZE; n++)
	{
		if (lpObj->Trade[n].IsItem() != 0 && lpObj->Trade[n].IsJewelOfHarmonyItem() != 0)
		{
			return 0;
		}
	}
