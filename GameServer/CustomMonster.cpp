...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	for(std::vector<CUSTOM_MONSTER_INFO>::iterator it= this->m_CustomMonsterInfo.begin();it != this->m_CustomMonsterInfo.end();it++)
	{
		...
		if(it->KillMessage != -1)
		{
			gNotice->GCNoticeSendToAll(0,0,0,0,0,0,it->KillMessage,lpTarget->Name);
		}
		...
		{
			gNotice->GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,it->RewardMessage,it->RewardCoin1,it->RewardCoin2,it->RewardCoin3);
		}
