...
bool CCustomGift::CommandGift(LPOBJ lpObj,char* arg) // OK
{
	...
	for(std::map<int,GIFT_LIST_INFO>::iterator it = this->m_GiftListInfo.begin(); it != this->m_GiftListInfo.end(); it++)
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,606);
			return 0;
		}

		if(it->second.MaxLevel != -1 && lpObj->Level > it->second.MaxLevel)
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,607);
			return 0;
		}

		if(it->second.MinReset != -1 && lpObj->Reset < it->second.MinReset)
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,608);
			return 0;
		}

		if(it->second.MaxReset != -1 && lpObj->Reset > it->second.MaxReset)
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,609);
			return 0;
		}
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,605);
...
void CCustomGift::DGCommandGiftRecv(SDHP_COMMAND_GIFT_RECV* lpMsg) // OK
{
	...
	if(lpMsg->Result == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,611);
		return;
	}
	...
	if(it != this->m_GiftListInfo.end())
	{
		...
		if(it->second.Coin1 > 0 || it->second.Coin2 > 0 || it->second.Coin3 > 0)
		{
			...
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,183,it->second.Coin1,it->second.Coin2,it->second.Coin3);
		}
		...
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,612);
