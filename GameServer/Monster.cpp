	...
	else if(lpObj->ActionState.Emotion == EMOTION_ATTACK)
	{
		...
		if(OBJECT_RANGE(lpObj->TargetNumber) != 0 && lpObj->PathStartEnd == 0)
		{
			if(gObjCalcDistance(lpObj,&gObj[lpObj->TargetNumber]) <= (lpObj->AttackType>=100?(lpObj->AttackRange+2):lpObj->AttackRange))
			{
				...
				{
					if(gMap[lpObj->Map].CheckAttr(gObj[lpObj->TargetNumber].X,gObj[lpObj->TargetNumber].Y,MAP_ATTR_SAFE) == 0)
					{
...
void CMonster::FindTarget(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if((lpObj->Class == 247 || lpObj->Class == 249 || gCustomGuard->GetGuard(lpObj->Class)) && gObj[lpObj->VpPlayer2[n].index].PKLevel > 4)
		{
...
void CMonster::GetXYToPatrol(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < 10;n++)
	{
		...
		{
			if(lpObj->Class == 247 || lpObj->Class == 249 || gCustomGuard->GetGuard(lpObj->Class))
			{
				if(gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_STAND) == 0 && gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_LOCK1) == 0 && gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_LOCK2) == 0)
				{
					lpObj->MTX = tx;
					lpObj->MTY = ty;
					lpObj->ActionState.Move = 1;
			...
			{
				if(gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_SAFE) == 0 && gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_STAND) == 0 && gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_LOCK1) == 0 && gMap[lpObj->Map].CheckAttr(tx,ty,MAP_ATTR_LOCK2) == 0)
				{
					lpObj->ActionState.Emotion = EMOTION_REST;
					lpObj->MTX = tx;
					lpObj->MTY = ty;
...
bool CMonster::GetXYToChase(LPOBJ lpObj) // OK
{
	...
	else
	{
		...
		{
			if(gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_SAFE) == 0 && gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_STAND) == 0 && gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_LOCK1) == 0 && gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_LOCK2) == 0)
			{
				lpObj->MTX = sx;
				lpObj->MTY = sy;
				return 1;
...
void CMonster::GetXYToOwner(LPOBJ lpObj) // OK
{
	...
	{
		if(gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_STAND) == 0 && gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_LOCK1) == 0 && gMap[lpObj->Map].CheckAttr(sx,sy,MAP_ATTR_LOCK2) == 0)
		{
			lpObj->MTX = sx;
			lpObj->MTY = sy;
			lpObj->ActionState.Move = 1;
...
void CMonster::DropItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class == 247 || lpObj->Class == 249 || lpObj->Class == 277 || lpObj->Class == 278 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 460 || lpObj->Class == 461 || lpObj->Class == 462 || lpObj->Class == 524 || lpObj->Class == 525 || lpObj->Class == 527 || lpObj->Class == 528 || lpObj->Class == 605 || lpObj->Class == 606 || lpObj->Class == 607 || gCustomGuard->GetGuard(lpObj->Class))
	{
		return;
	}
...
void CMonster::ProcessHit(LPOBJ lpObj,int aIndex) // OK
{
	...
	if(gMap[gObj[aIndex].Map].CheckAttr(gObj[aIndex].X,gObj[aIndex].Y,MAP_ATTR_SAFE) != 0)
	{
		return;
	}
...
int CMonster::MonsterCount(int index,int map,int x,int y,int tx,int ty) // OK
{
	int count = 0;
