...
void CCustomAttack::MainProc(LPOBJ lpObj) // OK
{
	...
	else
	{
		...
		if(this->m_CustomAttackMaxTimeLimit[lpObj->AccountLevel] > 0)
		{
			...
			{
				GCNewMessageSend(lpObj->Index,1,2,578);
...
void CCustomAttack::ReadCustomAttackInfo(char* section,char* path) // OK
{
	...
	this->m_CustomAttackExperienceRate[0] = GetPrivateProfileInt(section,"CustomAttackExperienceRate_AL0",0,path);
	this->m_CustomAttackExperienceRate[1] = GetPrivateProfileInt(section,"CustomAttackExperienceRate_AL1",0,path);
	this->m_CustomAttackExperienceRate[2] = GetPrivateProfileInt(section,"CustomAttackExperienceRate_AL2",0,path);
	this->m_CustomAttackExperienceRate[3] = GetPrivateProfileInt(section,"CustomAttackExperienceRate_AL3",0,path);
	...
	this->m_CustomAttackOfflineExperienceRate[0] = GetPrivateProfileInt(section,"CustomAttackOfflineExperienceRate_AL0",0,path);
	this->m_CustomAttackOfflineExperienceRate[1] = GetPrivateProfileInt(section,"CustomAttackOfflineExperienceRate_AL1",0,path);
	this->m_CustomAttackOfflineExperienceRate[2] = GetPrivateProfileInt(section,"CustomAttackOfflineExperienceRate_AL2",0,path);
	this->m_CustomAttackOfflineExperienceRate[3] = GetPrivateProfileInt(section,"CustomAttackOfflineExperienceRate_AL3",0,path);
...
void CCustomAttack::CustomAttackClose(int aIndex) // OK
{
	...
	GCNewMessageSend(lpObj->Index,1,2,572);
...
void CCustomAttack::CommandCustomAttack(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		GCNewMessageSend(lpObj->Index,1,2,573);
		return;
	}
	...
	else
	{
		...
		if(lpObj->Attack.AutoBuff == 0 && lpObj->Attack.SkillIndex == 0)
		{
			GCNewMessageSend(lpObj->Index,1,2,575);
			return;
		}
...
void CCustomAttack::CommandCustomAttackOffline(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Attack.Started == 0)
	{
		GCNewMessageSend(lpObj->Index,1,2,577);
		return;
	}

	//verifica se o auto reset esta ativo e desativa ao iniciar o offattack
	if (gServerInfo->m_CommandResetMove[lpObj->AccountLevel] != 0 && lpObj->AutoResetEnable == 1)
	{
...
int CCustomAttack::GetExperienceRate(LPOBJ lpObj) // OK
{
	if(this->m_CustomAttackSwitch == 0)
	{
		return 100;
