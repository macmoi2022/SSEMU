...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	if(gObj[SummonIndex].Map == MAP_CASTLE_SIEGE && gCastleSiegeSync->GetState() == CS_STATE_START)
	{
		...
		{
			if(gServerInfo->m_CastleSiegeDamageRate2 != 0 || lpObj->CastleJoinSide == CS_JOIN_SIDE_NONE || gObj[SummonIndex].CastleJoinSide == CS_JOIN_SIDE_NONE || lpObj->CastleJoinSide != gObj[SummonIndex].CastleJoinSide)
			{
				return 1;
			}
		}
...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		// FIX DL ATACAR SEM ARMA
		if (lpSkill->m_skill == SKILL_FIRE_BURST || lpSkill->m_skill == SKILL_FORCE || lpSkill->m_skill == SKILL_FIRE_SCREAM || lpSkill->m_skill == SKILL_ELECTRIC_SPARK || lpSkill->m_skill == SKILL_TWISTING_SLASH || lpSkill->m_skill == SKILL_RAGEFUL_BLOW || lpSkill->m_skill == SKILL_DEATH_STAB || lpSkill->m_skill == SKILL_FIRE_SLASH)
		{
			if (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() == 0)
			{
				return;
			...
			// FIX CHECK STAFF
			if (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index >= GET_ITEM(5, 0) && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index <= GET_ITEM(5, 512))
			{
				return;
			}
		}
		...
		//FIX ATTACK SEM ARCO
		if (lpSkill->m_skill == SKILL_PENETRATION || lpSkill->m_skill == SKILL_ICE_ARROW || lpSkill->m_skill == SKILL_TRIPLE_SHOT)
		{
			...
			if (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index == GET_ITEM(4, 15) && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index == GET_ITEM(4, 7) || lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index == GET_ITEM(4, 15) && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].IsItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index == GET_ITEM(4, 7) && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() == 0)
			{
				return;
			}

			if (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index >= GET_ITEM(0, 0) && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index <= GET_ITEM(3, 512))
			{
				return;
			}

			if (lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index >= GET_ITEM(5, 0) && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index <= GET_ITEM(6, 512))
			{
				return;
			}
		}
...
bool CSkillManager::SkillSummonParty(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		...
		for(int n=0;n < MAX_PARTY_USER;n++)
		{
			...
			{
				gNotice->GCNoticeSend(index,1,0,0,0,0,0,272);
				continue;
			}
			...
			gNotice->GCNoticeSend(index,1,0,0,0,0,0,273,gObj[index].SkillSummonPartyTime);
		}
	}
...
// Ajustado Skill invisivel do Castle Siege para que ele só use em si mesmo, ou em alguem da party.  Data: 09/08/2024
bool CSkillManager::SkillInvisibility(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	LPOBJ lpObj = &gObj[aIndex];

	// Verifica se o jogador está nos mapas Castle Siege ou Land of Trials
	if (lpObj->Map != MAP_CASTLE_SIEGE && lpObj->Map != MAP_LAND_OF_TRIALS)
	{
		return 0;
	}

	// Verifica se o alvo está dentro do range válido
	if (OBJECT_RANGE(bIndex) == 0)
	{
		return 0;
	}
	...
	// Se o alvo não é da mesma party, usa a skill no próprio jogador (aIndex)
	if (aIndex != bIndex && (lpObj->PartyNumber == -1 || lpObj->PartyNumber != lpTarget->PartyNumber))
	{
		lpTarget = lpObj; // Aplica a skill em si mesmo
	}

	// Verifica se o alvo (seja ele o próprio jogador ou membro da party) está dentro do range da skill
	if (this->CheckSkillRange(lpObj, lpSkill->m_index, lpObj->X, lpObj->Y, lpTarget->X, lpTarget->Y) == 0)
	{
		return 0;
	}

	// Reseta o contador de kills do jogador
	gObjSetKillCount(aIndex, 0);

	// Aplica o efeito de invisibilidade ao alvo
	gEffectManager->AddEffect(lpTarget, 0, EFFECT_INVISIBILITY, gServerInfo->m_InvisibilityTimeConstA, 0, 0, 0, 0);

	// Envia o ataque da skill ao cliente
	this->GCSkillAttackSend(lpObj, lpSkill->m_index, lpTarget->Index, 1);
...
void CSkillManager::ApplyLargeRingBlowerEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	{
		gEffectManager->AddEffect(lpTarget,0,this->GetSkillEffect(lpSkill->m_index),10,5,0,0,0);
	}
...
void CSkillManager::ApplyUpperBeastEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	{
		gEffectManager->AddEffect(lpTarget,0,this->GetSkillEffect(lpSkill->m_index),10,10,0,0,0);
	}
...
void CSkillManager::ApplyChainDriverEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	{
		gEffectManager->AddEffect(lpTarget,0,this->GetSkillEffect(lpSkill->m_index),10,0,0,0,0);
	}
...
void CSkillManager::ApplyEarthPrisonEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	{
		gEffectManager->AddEffect(lpTarget,0,this->GetSkillEffect(lpSkill->m_index),5,0,0,0,0);
	}
...
void CSkillManager::CGSkillAttackRecv(PMSG_SKILL_ATTACK_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gMap[lpTarget->Map].CheckAttr(lpTarget->X,lpTarget->Y,MAP_ATTR_SAFE) != 0)
	{
...
void CSkillManager::CGSkillTeleportAllyRecv(PMSG_SKILL_TELEPORT_ALLY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE)
	{
		...
		//Ajuste Coordenadas by LaMO
		Coordinates adjustedCoords = gCastleSiege->CheckTeleportCastleLimit(lpMsg->index, lpMsg->x, lpMsg->y);

		if (adjustedCoords.x != lpMsg->x) lpMsg->x = adjustedCoords.x;
		if (adjustedCoords.y != lpMsg->y) lpMsg->y = adjustedCoords.y;
		//Fim Ajuste
	}
	...
	if(lpSkill != 0)
	{
		if (gSkillManager->CheckSkillDelay(lpObj, lpSkill->m_index) == 0)
		{
			gMove->CGTeleportError(aIndex);
			return;
		}
