...
void CCustomLukeHelper::ProcState_EMPTY() // OK
{
	if(this->m_RemainTime > 0 && this->m_RemainTime < 300)
	{
		...
		if(this->m_MinutesLeft != minutes)
		{
			...
			gNotice->GCNoticeSendToAll(0,0,0,0,0,0,530,(this->m_MinutesLeft+1));
		}
	...
	if(this->m_RemainTime <= 0)
	{
		...
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,531,gMapManager->GetMapName(lpInfo->Map));
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,532);
...
void CCustomLukeHelper::ProcState_START() // OK
{
	if(this->m_RemainTime <= 0)
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,533);
...
bool CCustomLukeHelper::NpcLukeHelper(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	GCChatTargetSend(lpObj,lpNpc->Index,gMessage->GetText(534,lpObj->Lang));

	gNotice->GCNoticeSendToAll(0,0,0,0,0,0,535,lpObj->Name);
