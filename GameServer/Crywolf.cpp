...
void CCrywolf::ProcState_NOTIFY1() // OK
{
	...
	{
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(70,gServerInfo->m_ServerLang));
...
void CCrywolf::ProcState_NOTIFY2() // OK
{
	...
	if((GetTickCount()-this->m_CrywolfNotifyMsgStartTick) > 70000)
	{
		...
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(78,gServerInfo->m_ServerLang),(this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime()/1000));
		}
		else
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(79,gServerInfo->m_ServerLang),((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime()/1000)/60));
		}
...
void CCrywolf::ProcState_READY() // OK
{
	...
	if((GetTickCount()-this->m_CrywolfNotifyMsgStartTick) > 70000)
	{
		if(((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime()/1000)/60) == 0)
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(80,gServerInfo->m_ServerLang),(this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime()/1000));
		}
		else
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(81,gServerInfo->m_ServerLang),((this->m_StateTimeInfo[this->GetCrywolfState()].GetLeftTime()/1000)/60));
		}
...
void CCrywolf::ProcState_START()
{
	...
	{
		gCrywolfUtil->SendCrywolfUserAnyMsg(82);
	...
	{
		this->m_TurnUpBoss = 1;
		gCrywolfUtil->SendCrywolfUserAnyMsg(83);
		this->TurnUpBoss();
	}
...
void CCrywolf::SetState_NOTIFY2() // OK
{
	...
	GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(71,gServerInfo->m_ServerLang));
...
void CCrywolf::SetState_READY() // OK
{
	...
	GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(72,gServerInfo->m_ServerLang));
...
void CCrywolf::SetState_START() // OK
{
	...
	GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(73,gServerInfo->m_ServerLang));
	...
	if(gCrywolfAltar->GetContractedAltarCount() == 0)
	{
		this->SetOccupationState(1);
		this->SetState(CRYWOLF_STATE_END);
		gCrywolfUtil->SendCrywolfUserAnyMsg(74);
		return;
	}
...
void CCrywolf::SetState_END()
{
	...
	if(this->m_TurnUpBoss != 0 && OBJECT_RANGE(this->m_BossIndex) != 0 && gObj[this->m_BossIndex].Live != 0)
	{
		gCrywolfUtil->SendCrywolfUserAnyMsg(75);
		this->SetOccupationState(1);
	}
	...
	{
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(76,gServerInfo->m_ServerLang));
		this->SetOccupationState(1);
	}
	else
	{
		GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(77,gServerInfo->m_ServerLang));
		this->SetOccupationState(0);
	}
...
void CCrywolf::NotifyCrywolfPersonalRank() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnected(n) != 0 && gObj[n].Type == OBJECT_USER && gObj[n].Map == MAP_CRYWOLF)
		{
			...
			gNotice->GCNoticeSend(n,1,0,0,0,0,0,86,pMsg.rank);

			gNotice->GCNoticeSend(n,1,0,0,0,0,0,87,pMsg.experience);
		}
	}
...
void CCrywolf::CGCrywolfAltarContractRecv(PMSG_CRYWOLF_ALTAR_CONTRACT_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,105,260);
	}
...
void CCrywolf::CrywolfMonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(this->GetCrywolfState() == CRYWOLF_STATE_START && lpObj->Map == MAP_CRYWOLF && lpObj->CurrentAI != 0)
	{
		...
		{
			gCrywolfUtil->SendCrywolfUserAnyMsg(90,lpObj->GroupNumber,lpTarget->Name);
		}

		if(lpObj->Class == 349 && gObjIsConnected(lpTarget->Index) == 0)
		{
			gCrywolfUtil->SendCrywolfUserAnyMsg(89);
		}

		if(lpObj->Class == 349 && gObjIsConnected(lpTarget->Index) != 0)
		{
			gCrywolfUtil->SendCrywolfUserAnyMsg(88,lpTarget->Name);
		}

		if(gObjIsConnected(lpTarget->Index) != 0)
		{
			gNotice->GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,91,this->GetUserScore(lpTarget->Index,lpObj->Index,0));
		}
	}
