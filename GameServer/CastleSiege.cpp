...
void CCastleSiege::ProcState_SIGN1() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(111,gServerInfo->m_ServerLang));
...
void CCastleSiege::ProcState_SIGN2() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		if((GetTickCount()-this->m_MessageTime) > 1800000)
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(112,gServerInfo->m_ServerLang));
			this->m_MessageTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_IDLE3() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(113,gServerInfo->m_ServerLang));
			this->m_MessageTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_NOTIFY() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		if((GetTickCount()-this->m_MessageTime) > 1800000)
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(114,gServerInfo->m_ServerLang));
			this->m_MessageTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_READY() // OK
{
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		if((GetTickCount()-this->m_MessageTime) > 1800000)
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(115,gServerInfo->m_ServerLang));
			this->m_MessageTime = GetTickCount();
		}
		...
		if(this->m_RemainTime <= 300000)
		{
			...
			{
				GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(116,gServerInfo->m_ServerLang),(this->m_RemainTime/60000)+1);
...
void CCastleSiege::ProcState_START() // OK
{
	...
	if((GetTickCount()-this->m_TickCount) >= 1000)
	{
		...
		if((GetTickCount()-this->m_MessageTime) > 1800000)
		{
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(117,gServerInfo->m_ServerLang));
			this->m_MessageTime = GetTickCount();
		}
	}
...
void CCastleSiege::SetState_END(bool reset) // OK
{
	...
	GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(110,gServerInfo->m_ServerLang));
}
...
void CCastleSiege::ResetPlayerJoinSide() // OK
{
	for(int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		lpObj->CastleJoinSide = CS_JOIN_SIDE_NONE;
...
void CCastleSiege::CheckMiddleWinnerGuild() // OK
{
	...
	if(lpCrownAccess->CastleJoinSide >= CS_JOIN_SIDE_ATTACK_1 && lpSwitchUser1->CastleJoinSide >= CS_JOIN_SIDE_ATTACK_1 && lpSwitchUser2->CastleJoinSide >= CS_JOIN_SIDE_ATTACK_1)
	{
...
bool CCastleSiege::CheckCastleSiegeResult() // OK
{
	if(this->m_CastleOccupied == 1 && strcmp(this->m_CastleOwner,"") != 0)
	{
		if(strcmp(this->m_MiddleOwner,"") == 0 || strcmp(this->m_MiddleOwner,this->m_CastleOwner) == 0)
		{
			...
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(118,gServerInfo->m_ServerLang),this->m_CastleOwner);
		...
		else
		{
			...
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(119,gServerInfo->m_ServerLang),this->m_CastleOwner);
	...
	else
	{
		if(strcmp(this->m_MiddleOwner,"") == 0)
		{
			...
			GDGlobalNoticeSend(gMapServerManager->GetMapServerGroup(),0,0,0,0,0,0,gMessage->GetText(120,gServerInfo->m_ServerLang));
...
void CCastleSiege::RespawnPlayers(bool defense) // OK
{
	for(int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		// Ajustado para a validação do respawn ser apenas no mapa do CS e não em Land of Trials.  Data: 04/09/2024
		if (lpObj->Map != MAP_CASTLE_SIEGE)
		{
			continue;
		}

		// Ajustado para a validação do respawn ser apenas das atacantes, mantendo a defensora na mesma sem mover.  Data: 04/09/2024
		if (lpObj->CastleJoinSide == 1)
		{
			continue;
		}

		if(this->GetRespawnCoord(&lpObj->X,&lpObj->Y,lpObj->CastleJoinSide) == true)
		{
...
bool CCastleSiege::GetRespawnCoord(short* x,short* y,BYTE side) // OK
{
	...
	for(int n = 0; n < 100; n++)
	{
		...
		if(gMap[MAP_CASTLE_SIEGE].CheckAttr((*x),(*y),MAP_ATTR_LOCK1) == 0 && gMap[MAP_CASTLE_SIEGE].CheckAttr((*x),(*y),MAP_ATTR_LOCK2) == 0)
		{
			return 1;
		}
	}
...
void CCastleSiege::OperateMiniMapWork()
{
	...
	for (int iIDX = OBJECT_START_USER; iIDX < MAX_OBJECT; iIDX++)
	{
		...
		if(gObj[iIDX].CastleJoinSide == CS_JOIN_SIDE_NONE)
		{
			continue;
		}
...
void CCastleSiege::CGCastleSiegeRegisterRecv(int aIndex) // OK
{
	...
	if(this->GetState() != CS_STATE_SIGN1 || gServerInfo->m_CastleSiegeEvent == 0)
	{
	...
	if(lpObj->Level < gServerInfo->m_CastleSiegeRegisterMinMasterLevel)
	{
		this->GCCastleSiegeRegisterGuildResultSend(aIndex,5,"");
		return;
	}

	if(lpObj->Guild->GetMemberCount() < gServerInfo->m_CastleSiegeRegisterMinGuildMember)
	{
		this->GCCastleSiegeRegisterGuildResultSend(aIndex,8,"");
		return;
	}
...
void CCastleSiege::CGCastleSiegeMiniMapRequestRecv(int aIndex) // OK
{
	...
	if(lpObj->CastleJoinSide == CS_JOIN_SIDE_NONE)
	{
...
void CCastleSiege::GCCastleSiegeJoinSideSend(int aIndex) // OK
{
	...
	int state = CS_JOIN_SIDE_NONE;

	if(lpObj->Map == MAP_CASTLE_SIEGE && (this->GetState() == CS_STATE_START || this->GetState() == CS_STATE_END) && lpObj->CastleJoinSide > CS_JOIN_SIDE_NONE)
	{
		switch(lpObj->CastleJoinSide)
		{
			case CS_JOIN_SIDE_DEFENSE:
				state = CS_JOIN_SIDE_DEFENSE;
				gEffectManager->AddEffect(lpObj,0,EFFECT_GUILD_STATE1,0,0,0,0,0);
				break;
			case CS_JOIN_SIDE_ATTACK_1:
				state = CS_JOIN_SIDE_ATTACK_1;
				gEffectManager->AddEffect(lpObj,0,EFFECT_GUILD_STATE2,0,0,0,0,0);
				break;
			case CS_JOIN_SIDE_ATTACK_2:
				state = CS_JOIN_SIDE_ATTACK_2;
				gEffectManager->AddEffect(lpObj,0,EFFECT_GUILD_STATE3,0,0,0,0,0);
				break;
			case CS_JOIN_SIDE_ATTACK_3:
				state = CS_JOIN_SIDE_ATTACK_3;
...
void CCastleSiege::GDCastleSiegeSyncStateSend() // OK
{
	...
	if(this->m_CastleSiegeDataLoad == 2)
	{
		...
		else if(GetTickCount()-this->m_CastleSiegeDataLoadTime > 60000)
		{
			result = 1;
			this->m_CastleSiegeDataLoadTime = GetTickCount();
			LogAdd(LOG_RED,"[Castle Siege] Cannot connect DataServer");
		}
...
void CCastleSiege::NpcCastleSiegeCrown(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(lpObj->CastleJoinSide == CS_JOIN_SIDE_NONE || lpObj->CastleJoinSide == CS_JOIN_SIDE_DEFENSE || lpObj->CastleInvolved == 0)
	{
		return;
	}

	if(this->CheckUnionGuildMaster(lpObj->Index) == false)
	{
		return;
	}
	...
	if(this->GetRegCrownAvailable() == false)
	{
		return;
	}

	if(gObjIsConnected(this->GetCrownUserIndex()) == 0)
	{
		int aIndex = this->GetCrownSwitchUserIndex(217);
		int bIndex = this->GetCrownSwitchUserIndex(218);

		if(gObjIsConnected(aIndex) == 0 || gObjIsConnected(bIndex) == 0)
		{
			this->GCCastleSiegeCrownAccessStateSend(lpObj->Index,4);
			return;
		}
		...
		{
			this->GCCastleSiegeCrownAccessStateSend(lpObj->Index,0);
			this->SetCrownUserIndex(lpObj->Index);
			this->SetCrownAccessPosition((BYTE)lpObj->X,(BYTE)lpObj->Y);
			this->SetCrownAccessTickCount();
			this->GCCastleSiegeNotifyProgressSendToAll(0,lpObj->GuildName);
		}
	}
	else if(lpObj->Index != this->GetCrownUserIndex())
	{
		this->GCCastleSiegeCrownAccessStateSend(lpObj->Index,3);
	}
}

void CCastleSiege::NpcCastleSiegeCrownSwitch(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	if(gCastleSiegeSync->GetState() != CS_STATE_START)
	{
		return;
	...
	if(lpObj->CastleJoinSide == CS_JOIN_SIDE_NONE || lpObj->CastleJoinSide == CS_JOIN_SIDE_DEFENSE)
	{
		return;
	}
	...
	if(this->CheckGuardianStatueExist() == true)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,121);
		return;
	}

	int CrownUserIndex = this->GetCrownSwitchUserIndex(lpNpc->Class);

	if(gObjIsConnected(CrownUserIndex) == 0)
	{
		this->GCCastleSiegeSwitchStateSend(lpObj->Index,lpNpc->Index,-1,1);
		this->SetCrownSwitchUserIndex(lpNpc->Class,lpObj->Index);
	}
	else if(lpObj->Index != CrownUserIndex)
	{
		this->GCCastleSiegeSwitchStateSend(lpObj->Index,lpNpc->Index,CrownUserIndex,2);
	}
}

void CCastleSiege::NpcCastleSiegeGateSwitch(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	bool control = false;
	BYTE result = 0;
	int bIndex = -1;
	...
	{
		if(this->CheckCastleOwnerMember(lpObj->Index) == false && this->CheckCastleOwnerUnionMember(lpObj->Index) == false)
		{
			result = 4;
			control = false;
		}
	...
	if(control == true)
	{
		if(this->CheckCsGateAlive(lpNpc->CastleSwitchIndex) == true)
		{
			result = 1;
			bIndex = lpNpc->CastleSwitchIndex;
		}
		else
		{
			result = 2;
	...
	this->GCCastleSiegeGateStateSend(lpObj->Index,result,bIndex);
...
void CCastleSiege::NpcCastleSiegeGuard(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	pMsg.allowed = this->GetHuntZoneEnter();
	
	pMsg.CurrPrice = gCastleSiegeSync->GetTaxHuntZone(lpObj->Index,0);

	if(lpObj->Guild != 0)
	{
		if(this->CheckCastleOwnerMember(lpObj->Index) != 0)
		{
			if(lpObj->GuildRank == GUILD_STATUS_MASTER)
			{
				pMsg.result = 3;
		...
		else if(this->CheckCastleOwnerUnionMember(lpObj->Index) != 0)
		{
			pMsg.result = 2;
			pMsg.allowed = 0;
		}
...
Coordinates CCastleSiege::CheckTeleportCastleLimit(int aIndex, int x, int y) // By LaMO
{
	if (gObj[aIndex].X > 160 && gObj[aIndex].X < 191 && gObj[aIndex].Y > 188 && gObj[aIndex].Y < 217)
	{
		/*Limites
			162 190
			189 190
