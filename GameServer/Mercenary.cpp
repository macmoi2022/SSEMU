...
bool CMercenary::CreateMercenary(int aIndex,int MonsterClass,int x, int y) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,128);
		return 0;
	}
	...
	if(MonsterClass == 286 || MonsterClass == 287)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,129);
			return 0;
		}
		if(lpObj->GuildRank != GUILD_STATUS_OFFICE_MASTER && lpObj->GuildRank != GUILD_STATUS_MASTER)
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,130);
			return 0;
		}
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,131);
		return 0;
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,133);
		return 0;
	}
	...
	lpMonster->CastleJoinSide = CS_JOIN_SIDE_DEFENSE;

	gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,132);
