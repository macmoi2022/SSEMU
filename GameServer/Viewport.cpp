...
void CViewport::CreateViewportPlayer(int aIndex) // OK
{
	...
	{
		if (gObj[Index].Connected == OBJECT_ONLINE)
		{
...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		// LaMO
		if (gEffectManager->CheckEffect(lpTarget, EFFECT_INVISIBILITY) != 0 && (lpTarget->Authority == AUTHORITY_GAME_MASTER || lpTarget->Authority == AUTHORITY_ADMINISTRATOR))
		{
			continue;
		}
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	for (int Index = OBJECT_START_USER; Index < MAX_OBJECT; ++Index)
	{
		if (gObj[Index].Connected == OBJECT_ONLINE)
		{
			Patent.GCReqRankLevelUser(lpObj->Index, Index);
		}
	}
