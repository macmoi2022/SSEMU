...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if (lpObj->HackSkillBlockTime > 0)
	{
		if (lpObj->HackSkillBlockTime < GetTickCount())
		{
			lpObj->HackSkillBlockTime = 0;
		}
		return 0;
	}


	if(gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) != 0 || gMap[lpTarget->Map].CheckAttr(lpTarget->X,lpTarget->Y,MAP_ATTR_SAFE) != 0)
	{
		return 0;
	}
	...
	if(AB_MAP_RANGE(lpObj->Map) || AB_MAP_RANGE(lpTarget->Map))
	{
		if(gArcaBattle->GetState() == AB_STATE_START)
		{
			if(lpObj->Type == OBJECT_USER)
			{
				if(lpTarget->Type == OBJECT_USER && gArcaBattle->CheckPlayerTarget(lpObj,lpTarget) == 0)
				{
					return 0;
				}

				if(lpTarget->Type == OBJECT_MONSTER && gArcaBattle->CheckMonsterTarget(lpObj,lpTarget) == 0)
				{
					return 0;
				}
			}
	...
	if(lpTarget->Map == MAP_CASTLE_SIEGE && lpTarget->Type == OBJECT_MONSTER && (lpTarget->Class == 277 || lpTarget->Class == 283 || lpTarget->Class == 288))
	{
	...
	if (DeathEvent.Attack(lpObj, lpTarget) == false)
	{
		return 0;
	}

	if (TheftEvent.Attack(lpObj, lpTarget) == false)
	{
		return 0;
	}

	if (SobreEvent.Attack(lpObj, lpTarget) == false)
	{
		return 0;
	}

	if (DuelEvent.Attack(lpObj, lpTarget) == false)
	{
		return 0;
	}

	if (PairEvent.Attack(lpObj, lpTarget) == false)
	{
		return 0;
	}
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			{
				if(lpObj->CastleJoinSide == CS_JOIN_SIDE_NONE || lpTarget->CastleJoinSide == CS_JOIN_SIDE_NONE || lpObj->CastleJoinSide != lpTarget->CastleJoinSide)
				{
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiege->GetState() == CS_STATE_START && lpObj->CastleJoinSide > CS_JOIN_SIDE_NONE)
			{
				CheckSelfDefense = 0;
			}
			...
			if(gArcaBattle->GetState() == AB_STATE_START && AB_MAP_RANGE(lpObj->Map) != 0 && AB_MAP_RANGE(lpTarget->Map) != 0)
			{
				CheckSelfDefense = 0;
			}
	...
	if(damage > 0)
	{
		...
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			if(stuck == true)
			{
				...
				{
					if (gCustomPet->PetIsBlockStuck(lpTarget->Inventory[8].m_Index) == false)
					{
						flag = 1;
					}
				}
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	if (gCustomPet->CheckCustomPetByItem(lpItem->m_Index) != 0) // CustomPets
	{
		if (gCustomPet->PetIsInmortal(lpItem->m_Index) == true)
		{
			if (lpItem->m_Durability < 50.0f)
			{
				lpItem->m_Durability = 255.0f;
				LogAdd(LOG_BLUE, "[CustomPet][%s][%s] Pet:%d Life Recovery:%f", lpObj->Account, lpObj->Name, lpItem->m_Index, lpItem->m_Durability);
			}
		}
	}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	//ADICIONADO FUNÇÃO PARA PERMITIR ATACAR DURANTE O EVENTO
	if (DS_MAP_RANGE(lpObj->Map) != 0 && DS_MAP_RANGE(lpTarget->Map) != 0 && gServerInfo->m_DevilSquareDuelEnable == 0)
	{
		return 0;
	}
	//ADICIONADO FUNÇÃO PARA PERMITIR ATACAR DURANTE O EVENTO
	if (BC_MAP_RANGE(lpObj->Map) != 0 && BC_MAP_RANGE(lpTarget->Map) != 0 && gServerInfo->m_BloodCastleDuelEnable == 0)
	{
		return 0;
	}
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE && lpTarget->Map == MAP_CASTLE_SIEGE)
	{
		...
		{
			if(lpObj->CastleJoinSide > CS_JOIN_SIDE_NONE && lpTarget->CastleJoinSide > CS_JOIN_SIDE_NONE)
			{
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		if(DualHandWeapon != 0)
		{
			if (gServerInfo->m_TwoWeaponsDamageEnable != 0)
			{
				DamageMin = (((lpObj->PhysiDamageMinRight + lpObj->PhysiDamageMinLeft + SkillDamageMin) * gServerInfo->m_TwoWeaponsDamageMinConstA[lpObj->Class]) / 100);//ogocx
				DamageMax = (((lpObj->PhysiDamageMaxRight + lpObj->PhysiDamageMaxLeft + SkillDamageMax) * gServerInfo->m_TwoWeaponsDamageMaxConstA[lpObj->Class]) / 100);//ogocx
			}
			else
			{
