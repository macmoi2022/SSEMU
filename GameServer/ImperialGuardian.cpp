		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_NotifyTime = lpReadFile->GetNumber();
				...
				else if(section == 1)
				{
					...
					this->m_DefaultMonsterAIUnitNumber = lpReadFile->GetNumber();

					this->m_SupportMonsterAIUnitNumber = lpReadFile->GetAsNumber();
				}
				else if(section == 2)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
					...
					info.Life = lpReadFile->GetAsFloatNumber();

					info.DamageMin = lpReadFile->GetAsFloatNumber();

					info.DamageMax = lpReadFile->GetAsFloatNumber();

					info.Defense = lpReadFile->GetAsFloatNumber();

					info.AttackSuccessRate = lpReadFile->GetAsFloatNumber();

					info.DefenseSuccessRate = lpReadFile->GetAsFloatNumber();
...
void CImperialGuardian::ClearMonsters() // OK
{
	#if(GAMESERVER_UPDATE>=501)

	for(int n=OBJECT_START_MONSTER;n < MAX_OBJECT_MONSTER;n++)
	{
		if(gObjIsConnected(n) != 0 && gObj[n].Map == this->m_Map && (gObj[n].Class == 523 || gObj[n].Class == 526))
		{
			gObjDel(n);
		}
	}
...
void CImperialGuardian::SetSideGate() // OK
{
	...
	int index = gMonster->AddMonster(this->m_Map);
	...
	if(gMonster->SetMonster(index,((this->m_Level==3)?528:525)) == 0)
	{
		gObjDel(index);
		return;
	}
	...
	lpObj->Life = (float)this->GetMonsterLife(lpObj->Life);
	lpObj->MaxLife = (float)this->GetMonsterLife(lpObj->MaxLife);
...
void CImperialGuardian::SetLockGate() // OK
{
	...
	if(gMonster->SetMonster(index,((this->m_Level==3)?527:524)) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CImperialGuardian::SetSideBoss(int MonsterClass) // OK
{
	...
	if(gMonster->SetMonster(index,MonsterClass) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CImperialGuardian::SetMonsters() // OK
{
	#if(GAMESERVER_UPDATE>=501)
	...
	for(int n=0;n < gMonsterSetBase->m_count;n++)
	{
		...
		if(lpInfo->Type != 4 || (lpInfo->MonsterClass != 523 && lpInfo->MonsterClass != 526) || lpInfo->Map != this->m_Map)
		{
			continue;
		}
...
void CImperialGuardian::NpcJerryTheAdviser(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(lpObj->Level < 15)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,151);
		return;
	}
...
void CImperialGuardian::DropStoneStatueItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	#if(GAMESERVER_UPDATE>=501)
	...
	{
		case 0:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,0,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
		case 1:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,1,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
		case 2:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,2,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
		case 3:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,3,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
		case 4:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,4,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
		default:
			gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_IMPERIAL_GUARDIAN,5,2,lpTarget,lpObj->Map,lpObj->X,lpObj->Y);
			break;
	}
...
void CImperialGuardian::NoticeSendToAll(int type,int message,...) // OK
{
	#if(GAMESERVER_UPDATE>=501)
	...
	for(int n=0;n < MAX_IG_USER;n++)
	{
		if(OBJECT_RANGE(this->m_User[n].Index) != 0)
		{
			...
			gNotice->GCNoticeSend(this->m_User[n].Index,type,0,0,0,0,0,"%s",buff);
		}
	}
...
void CImperialGuardian::CGImperialGuardianEnterRecv(PMSG_IMPERIAL_GUARDIAN_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,718,gServerInfo->m_ImperialGuardianMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
