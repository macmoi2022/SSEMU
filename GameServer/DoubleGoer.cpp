		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 1)
				{
					...
					this->m_MonsterAIUnitNumber[0] = lpReadFile->GetNumber();

					this->m_MonsterAIUnitNumber[1] = lpReadFile->GetAsNumber();

					this->m_MonsterAIUnitNumber[2] = lpReadFile->GetAsNumber();

					this->m_MonsterAIUnitNumber[3] = lpReadFile->GetAsNumber();
				}
				else if(section == 2)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
					...
					info.Level[0] = lpReadFile->GetAsFloatNumber();

					info.Level[1] = lpReadFile->GetAsFloatNumber();

					info.Level[2] = lpReadFile->GetAsFloatNumber();

					info.Level[3] = lpReadFile->GetAsFloatNumber();

					info.Level[4] = lpReadFile->GetAsFloatNumber();

					info.Life[0] = lpReadFile->GetAsFloatNumber();

					info.Life[1] = lpReadFile->GetAsFloatNumber();

					info.Life[2] = lpReadFile->GetAsFloatNumber();

					info.Life[3] = lpReadFile->GetAsFloatNumber();

					info.Life[4] = lpReadFile->GetAsFloatNumber();

					info.Damage[0] = lpReadFile->GetAsFloatNumber();

					info.Damage[1] = lpReadFile->GetAsFloatNumber();

					info.Damage[2] = lpReadFile->GetAsFloatNumber();

					info.Damage[3] = lpReadFile->GetAsFloatNumber();

					info.Damage[4] = lpReadFile->GetAsFloatNumber();

					info.Defense[0] = lpReadFile->GetAsFloatNumber();

					info.Defense[1] = lpReadFile->GetAsFloatNumber();

					info.Defense[2] = lpReadFile->GetAsFloatNumber();

					info.Defense[3] = lpReadFile->GetAsFloatNumber();

					info.Defense[4] = lpReadFile->GetAsFloatNumber();
...
void CDoubleGoer::SetSlaughterer() // OK
{
	...
	if(gMonster->SetMonster(index,530) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CDoubleGoer::SetMadSlaughterer() // OK
{
	...
	if(gMonster->SetMonster(index,529) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CDoubleGoer::SetIceWalker() // OK
{
	...
	if(gMonster->SetMonster(index,531) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CDoubleGoer::SetSilverRewardChest(int px,int py) // OK
{
	...
	if(gMonster->SetMonster(index,541) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CDoubleGoer::SetGoldenRewardChest(int px,int py) // OK
{
	...
	if(gMonster->SetMonster(index,542) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CDoubleGoer::NpcSartiganTheAngel(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(lpObj->Level < 10)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,148);
		return;
	}
...
void CDoubleGoer::NoticeSendToAll(int type,int message,...) // OK
{
	#if(GAMESERVER_UPDATE>=501)
...
void CDoubleGoer::CGDoubleGoerEnterRecv(PMSG_DOUBLE_GOER_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,716,gServerInfo->m_DoubleGoerMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
