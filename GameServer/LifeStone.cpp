...
bool CLifeStone::CreateLifeStone(int aIndex) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,122);
		return 0;
	}

	if(lpObj->CastleJoinSide < CS_JOIN_SIDE_ATTACK_1)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,123);
		return 0;
	}

	if(lpObj->Guild->LifeStone != 0)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,124);
		return 0;
	}

	if(px > 150 && px < 210 && py > 175 && py < 230)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,125);
		return 0;
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,127);
		return 0;
	}
	...
	gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,126);
