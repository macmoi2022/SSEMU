...
void CReconnect::SetReconnectInfo(LPOBJ lpObj) // OK
{
	...
	info.ShopOpen = lpObj->Store.Open;

	info.ShopOffline = lpObj->Store.Offline;

	info.ShopCustomType = lpObj->Store.CustomType;
	...
	for(int n = INVENTORY_EXT4_SIZE; n < INVENTORY_FULL_SIZE; n++)
	{
		...
		info.ShopValue[n-INVENTORY_EXT4_SIZE] = lpObj->Inventory[n].m_PShopValue;
	}
...
void CReconnect::GetReconnectInfo(LPOBJ lpObj) // OK
{
	...
	if(it != this->m_ReconnectInfo.end())
	{
		...
		this->ResumeStore(lpObj,&it->second);
...
void CReconnect::ResumeStore(LPOBJ lpObj,RECONNECT_INFO* lpInfo) // OK
{
	...
	if(lpInfo->ShopOpen != 0 && lpInfo->ShopOffline == 1)
	{
		int count = 0;
		...
		for(int n = INVENTORY_EXT4_SIZE; n < INVENTORY_FULL_SIZE; n++)
		{
			...
			if(lpInfo->ShopValue[n-INVENTORY_EXT4_SIZE] < 1)
			{
				continue;
			}

			lpObj->Inventory[n].m_PShopValue = lpInfo->ShopValue[n-INVENTORY_EXT4_SIZE];
		...
		if(count > 0)
		{
			lpObj->Store.Open = lpInfo->ShopOpen;

			gCustomStore->OpenCustomStore(lpObj,lpInfo->ShopCustomType);
...
void CReconnect::ResumeOffline(RECONNECT_INFO* lpInfo) // OK
{
	...
	if(gCustomAttack->m_CustomAttackOfflineSwitch == 0 && gCustomStore->m_CustomStoreOfflineSwitch == 0 && gServerInfo->m_HelperOfflineSwitch == 0)
	{
		return;
	}

	if(lpInfo->AttackOffline == 0 && lpInfo->ShopOffline == 0 && lpInfo->HelperOffline == 0)
	{
		return;
	}

	if(lpInfo->ShopOffline > 1)
	{
		return;
	}
