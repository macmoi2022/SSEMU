...
void CItemDrop::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.DropRate[0] = lpReadFile->GetAsNumber();

			info.DropRate[1] = lpReadFile->GetAsNumber();

			info.DropRate[2] = lpReadFile->GetAsNumber();

			info.DropRate[3] = lpReadFile->GetAsNumber();
...
int CItemDrop::DropItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(std::vector<ITEM_DROP_INFO>::iterator it=this->m_ItemDropInfo.begin();it != this->m_ItemDropInfo.end();it++)
	{
		...
		if ((DropRate = it->DropRate[lpTarget->AccountLevel]) == -1 || (GetLargeRand() % 1000000) < (DropRate = this->GetItemDropRate(lpObj, lpTarget, it->ItemIndex, it->DropRate[lpTarget->AccountLevel])))
		{
