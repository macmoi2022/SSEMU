...
int CKanturuMonsterMng::GetPosition(int index,short map,short* x,short* y) // OK
{
	...
	if(gMap[map].CheckAttr(this->m_SetBaseInfo[index].X,this->m_SetBaseInfo[index].Y,MAP_ATTR_SAFE) == 0 && gMap[map].CheckAttr(this->m_SetBaseInfo[index].X,this->m_SetBaseInfo[index].Y,MAP_ATTR_LOCK1) == 0 && gMap[map].CheckAttr(this->m_SetBaseInfo[index].X,this->m_SetBaseInfo[index].Y,MAP_ATTR_LOCK2) == 0)
	{
	...
	for(int n=0;n < 100;n++)
	{
		...
		if(gMap[map].CheckAttr(px,py,MAP_ATTR_SAFE) == 0 && gMap[map].CheckAttr(px,py,MAP_ATTR_LOCK1) == 0 && gMap[map].CheckAttr(px,py,MAP_ATTR_LOCK2) == 0)
		{
			(*x) = px;
			(*y) = py;
			return 1;
