...
void CMuCashShop::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			info.CategoryIndex = lpReadFile->GetNumber();
			...
			info.ItemDurability = lpReadFile->GetAsNumber();
			...
			info.ItemHarmonyOption = lpReadFile->GetAsNumber();

			info.ItemOptionEx = lpReadFile->GetAsNumber();

			info.ItemDuration = lpReadFile->GetAsNumber();

			info.CoinValue = lpReadFile->GetAsNumber();
