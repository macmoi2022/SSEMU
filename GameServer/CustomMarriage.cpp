...
void CCustomMarriage::CommandPropuse(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,650);
		return;
	}

	if(lpObj->Interface.use != 0 || lpTarget->Interface.use != 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,651);
		return;
	}

	if(lpObj->Married != 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,652,lpObj->MarryName);
		return;
	}

	if(lpTarget->Married != 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,653,lpTarget->Name);
		return;
	}

	if(this->CheckSex(lpObj) == this->CheckSex(lpTarget) && this->m_CustomMarriageHomoSexual == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,654);
		return;
	}

	if(this->CheckPosition(lpObj,lpTarget) == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,655);
		return;
	}

	if(this->CheckWeddingRequerimentItem(lpObj) == 0 || this->CheckWeddingRequerimentItem(lpTarget) == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,656,this->m_WeddingItemCount,gItemManager->GetItemName(this->m_WeddingItemIndex,this->m_WeddingItemLevel));
		return;
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,657);
	
	gNotice->GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,658,lpObj->Name);
...
void CCustomMarriage::CommandAccept(LPOBJ lpObj) // OK
{
	...
	if(GetTickCount() - lpObj->MarryRequestTime > 60000)
	{
		...
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,659);
		return;
	}
	...
	if(lpTarget == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,660);
		return;
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,661,lpTarget->Name);
	
	gNotice->GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,661,lpObj->Name);

	if(this->m_CustomMarriageAnnounce != 0)
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,662,lpObj->Name,lpTarget->Name);
	}
...
void CCustomMarriage::CommandDivorce(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Interface.use != 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,663);
		return;
	}

	if(this->CheckDivorceRequerimentItem(lpObj) == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,664,this->m_DivorseItemCount,gItemManager->GetItemName(this->m_DivorseItemIndex,this->m_DivorseItemLevel));
		return;
	}
	...
	else
	{
		...
		gNotice->GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,665);
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,665);
...
void CCustomMarriage::CommandTrackMarry(LPOBJ lpObj) // OK
{
	...
	if(lpTarget == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,666);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,667);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,668);
		return;
	}
