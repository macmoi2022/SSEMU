...
void CChaosCastle::ProcState_EMPTY(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,728,(lpLevel->MinutesLeft+1));
			}
		}
	}
...
void CChaosCastle::ProcState_READY(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0)
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,16,(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	if(lpLevel->RemainTime <= 0)
	{
		if(lpLevel->Level == 0) { gNotice->GCNoticeSendToAll(0,0,0,0,0,0,17); }
		this->NoticeSendToAll(lpLevel,1,18,(lpLevel->Level+1),this->m_NotifyTime);
...
void CChaosCastle::ProcState_STAND(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		this->NoticeSendToAll(lpLevel,0,21);
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,19);
...
void CChaosCastle::ProcState_START(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	if(lpLevel->RemainTime <= 0)
	{
		this->NoticeSendToAll(lpLevel,0,20);
...
void CChaosCastle::GiveUserRewardMoney(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	this->NoticeSendToAll(lpLevel,1,724,gObj[WinnerIndex].Name,Money);
}
...
void CChaosCastle::CheckUserPosition(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	for(int n = 0; n < MAX_CC_USER; n++)
	{
		...
		if(gMap[lpLevel->Map].CheckAttr(gObj[lpLevel->User[n].Index].X,gObj[lpLevel->User[n].Index].Y,MAP_ATTR_LOCK2) != 0)
		{
...
void CChaosCastle::CheckMonsterPosition(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	for(int n = 0; n < MAX_CC_MONSTER; n++)
	{
		...
		if(gMap[lpLevel->Map].CheckAttr(gObj[lpLevel->MonsterIndex[n]].X,gObj[lpLevel->MonsterIndex[n]].Y,MAP_ATTR_LOCK2) != 0)
		{
...
void CChaosCastle::CheckItemPosition(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	for(int n = 0; n < MAX_MAP_ITEM; n++)
	{
		...
		if(gMap[lpLevel->Map].CheckAttr(gMap[lpLevel->Map].m_Item[n].m_X,gMap[lpLevel->Map].m_Item[n].m_Y,MAP_ATTR_LOCK2) != 0)
		{
...
bool CChaosCastle::GetFreePosition(CHAOS_CASTLE_LEVEL* lpLevel,int* ox,int* oy) // OK
{
	for(int x = ((*ox) - 8); x < ((*ox)+8); x++)
	{
		...
		{
			if(gMap[lpLevel->Map].CheckAttr(x,y,MAP_ATTR_STAND) == 0 && gMap[lpLevel->Map].CheckAttr(x,y,MAP_ATTR_LOCK1) == 0 && gMap[lpLevel->Map].CheckAttr(x,y,MAP_ATTR_LOCK2) == 0)
			{
				(*ox) = x;
				(*oy) = y;
				return 1;
...
void CChaosCastle::NoticeSendToAll(CHAOS_CASTLE_LEVEL* lpLevel,int type,int message,...) // OK
{
	char buff[256] = {0};
	...
	for(int n = 0; n < MAX_CC_USER; n++)
	{
		if(OBJECT_RANGE(lpLevel->User[n].Index) != 0)
		{
			...
			gNotice->GCNoticeSend(lpLevel->User[n].Index,type,0,0,0,0,0,"%s",buff);
		}
	}
}
...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCNewMessageSend(aIndex,0,0,22);
		return;
	}
	...
	{
		GCNewMessageSend(aIndex,0,0,714,gServerInfo->m_ChaosCastleMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
	...
	{
		GCNewMessageSend(aIndex,0,0,159);
		return;
	}
