...
void CCustomBuyVip::CommandBuyVip(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,674);
		return;
	}

	if(it->second.AccountLevel != -1 && it->second.AccountLevel > lpObj->AccountLevel)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,675);
		return;
	}

	if(it->second.Money > lpObj->Money)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,676,it->second.Money);
		return;
	}

	if(it->second.Coin1 > lpObj->Coin1)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,677,it->second.Coin1);
		return;
	}

	if(it->second.Coin2 > lpObj->Coin2)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,678,it->second.Coin2);
		return;
	}

	if(it->second.Coin3 > lpObj->Coin3)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,679,it->second.Coin3);
		return;
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,680);
