...
{
	if(this->IsParty(lpObj->PartyNumber) == 0 || this->IsParty(lpTarget->PartyNumber) == 0)
	{
		return 0;
	}
...
int CParty::GetMemberHighestLevel(int index,int map,int x,int y) // OK
{
	int PartyTopLevel = 0;
	...
	if(this->IsParty(index) != 0)
	{
		for(int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			if(lpObj->Map == map && gObjCalcDistance(lpObj,x,y) < MAX_PARTY_DISTANCE)
			{
				if(gMasterSkillTree->CheckMasterLevel(lpObj) == 0)
				{
					PartyTopLevel = ((lpObj->Level>PartyTopLevel)?lpObj->Level:PartyTopLevel);
				}
				else
				{
					PartyTopLevel = (((lpObj->Level+lpObj->MasterLevel)>PartyTopLevel)?(lpObj->Level+lpObj->MasterLevel):PartyTopLevel);
				}
			}
		}
	}

	return PartyTopLevel;
}

int CParty::GetMemberHighestPKLevel(int index) // OK
{
	int PartyTopPKLevel = 3;
	...
	if(this->IsParty(index) != 0)
	{
		for(int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			if(gObj[this->m_PartyInfo[index].Index[n]].PKLevel > PartyTopPKLevel)
			{
				PartyTopPKLevel = gObj[this->m_PartyInfo[index].Index[n]].PKLevel;
			}
		}
	}

	return PartyTopPKLevel;
}
...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if(AB_MAP_RANGE(lpObj->Map) != 0 && (gArcaBattle->GetState() == AB_STATE_PARTY || gArcaBattle->GetState() == AB_STATE_START) && lpObj->GuildIndex != lpTarget->GuildIndex)
	{
		this->GCPartyResultSend(aIndex,0);
		return;
	}
