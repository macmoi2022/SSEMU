...
int LuaGetObjectGuildUnionNumber(lua_State* L) // OK
{
	...
	{
		LogAdd(LOG_RED, "Usuario sem guild");
		return 0;
	}
...
int LuaGetObjectGuildUnionName(lua_State* L) // OK
{
	...
	//ocorre o crash se o player não tiver guild
...
int LuaGetObjectOfflineFlag(lua_State* L) // OK
{
	...
	if(gObj[aIndex].Attack.Offline != 0 || gObj[aIndex].Helper.Offline != 0 || gObj[aIndex].Store.Offline != 0)
	{
		lua_pushinteger(L,1);
	}
	else
...
int LuaMonsterCreate(lua_State* L) // OK
{
	...
	int index = gMonster->AddMonster(bValue);
	...
	if(gMonster->SetMonster(index,aValue) == 0)
	{
		gObjDel(index);
		return 0;
	}
...
int LuaMonsterSummonCreate(lua_State* L) // OK
{
	...
	gMonster->SetMonster(index,aValue);
...
int LuaPostSend(lua_State* L) // OK
{
	...
	{
		case 0:
			PostMessage_1((char*)aString,gMessage->GetMessage(bValue),(char*)bString);
			break;
		case 1:
			PostMessage_2((char*)aString,gMessage->GetMessage(bValue),(char*)bString);
			break;
		case 2:
			PostMessage_3((char*)aString,gMessage->GetMessage(bValue),(char*)bString);
			break;
		case 3:
			PostMessage_4((char*)aString,gMessage->GetMessage(bValue),(char*)bString);
			break;
	}
