...
void CMap::Load(char* path,int map) // OK
{
	...
	if(width < 0 || width >= MAX_MAP_SIZE || height < 0 || height >= MAX_MAP_SIZE)
	{
...
{
	if(this->CheckAttr(x,y,MAP_ATTR_STAND) != 0)
	{
		return false;
	}

	if(this->CheckAttr(x,y,MAP_ATTR_LOCK1) != 0)
	{
		return false;
	}

	if(this->CheckAttr(x,y,MAP_ATTR_LOCK2) != 0)
	{
		return false;
	}
...
void CMap::SetStandAttr(int x,int y) // OK
{
	...
	this->m_MapAttr[(y*this->m_height)+x] |= MAP_ATTR_STAND;
}
...
void CMap::GetMapRandomPos(short* ox,short* oy,int size) // OK
{
	for(int n=0;n < 30;n++)
	{
		...
		if(this->CheckAttr(x,y,MAP_ATTR_LOCK1) == 0 && this->CheckAttr(x,y,MAP_ATTR_LOCK2) == 0)
		{
			(*ox) = x;
			(*oy) = y;
			return;
...
bool CMap::CheckItemGive(int aIndex,int index) // OK
{
	...
	if(loot == 0)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,260);
		return false;
	}
...
bool CMap::CheckWall(int sx1, int sy1, int sx2, int sy2) // OK
{
	...
	do
	{
		if ((this->m_MapAttr[Index] & MAP_ATTR_LOCK1) == MAP_ATTR_LOCK1)
		{
			return false;
		}

		if ((this->m_MapAttr[Index] & MAP_ATTR_LOCK3) == MAP_ATTR_LOCK3)
		{
			return false;
		}
