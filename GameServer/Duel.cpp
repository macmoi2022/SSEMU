...
bool CDuel::AddDuelArena(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(int n=0;n < MAX_DUEL_ARENA;n++)
	{
		...
		if(lpInfo->State == DA_STATE_EMPTY)
		{
			...
			if(gServerInfo->m_DuelArenaAnnounceSwitch != 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,174,lpObj->Name,lpTarget->Name);}
...
void CDuel::CheckDuelScore(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		gNotice->GCNoticeSendToAll(0,0,0,0,0,0,721,lpObj->Name,lpObj->DuelScore,lpTarget->DuelScore,lpTarget->Name);
	}
	...
	if(gServerInfo->m_DuelArenaAnnounceSwitch == 2)
	{
		...
		if(lpInfo != 0)
		{
			gNotice->GCNoticeSendToAll(0,0,0,0,0,0,175,lpInfo->Index,lpObj->Name,lpObj->DuelScore,lpTarget->DuelScore,lpTarget->Name);
		}
	}
	...
	if(lpObj->DuelScore >= gServerInfo->m_DuelMaxScore)
	{
		...
		if(gServerInfo->m_DuelAnnounceSwitch != 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,722,lpObj->Name,lpTarget->Name);}
		...
		if(lpInfo != 0)
		{
			...
			if(gServerInfo->m_DuelArenaAnnounceSwitch != 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,176,lpObj->Name,lpTarget->Name);}
...
void CDuel::CGDuelStartRecv(PMSG_DUEL_START_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,167);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,160);
		return;
	}

	LPOBJ lpTarget = &gObj[bIndex];

	if (lpTarget->InEvent == true || TheftEvent.PlayerStruct[lpTarget->Index]._InEvent == true || DeathEvent.PlayerStruct[lpTarget->Index]._InEvent == true || RoletaEvent.PlayerStruct[lpTarget->Index]._InEvent == true)
	{
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,168);
			return;
		}
	}
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,169);
		return;
	}
	...
	for(int n=0;n < MAX_SELF_DEFENSE;n++)
	{
		...
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,170);
			return;
		}

		if(lpTarget->SelfDefense[n] >= 0)
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,171);
			return;
		}
	}
...
void CDuel::CGDuelOkRecv(PMSG_DUEL_OK_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->flag == 0)
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,172);
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
	...
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,173);
		this->ResetDuel(lpObj);
		this->ResetDuel(lpTarget);
		this->GCDuelStartSend(bIndex,0,aIndex);
		return;
	...
	if(gServerInfo->m_DuelAnnounceSwitch != 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,720,lpObj->Name,lpTarget->Name);}
