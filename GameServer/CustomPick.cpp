...
void CCustomPick::ReadCustomPickInfo(char* section,char* path) // OK
{
	...
	this->m_PickRange = GetPrivateProfileInt(section, "CustomPickRange", 0, path);
}
...
void CCustomPick::MainProc(LPOBJ lpObj) // OK
{
	...
	if(this->m_CustomPickMaxTime[lpObj->AccountLevel] > 0)
	{
		...
		{
			gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,458);
	...
	for(int n=0;n<MAX_MAP_ITEM;n++)
	{
		...
		if(gViewport->CheckViewportObjectPosition(lpObj->Index,lpObj->Map,lpItem->m_X,lpItem->m_Y, this->m_PickRange) != 0)
		{
...
void CCustomPick::CommandCustomPick(LPOBJ lpObj,char* arg) // OK
{
	...
	if(gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) != 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,450);
		return;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,451);
		return;
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,452,lpObj->Pick.ItemCount,lpObj->Pick.PickMoney,lpObj->Pick.PickJewel,lpObj->Pick.PickExc,lpObj->Pick.PickSet);
...
void CCustomPick::CommandCustomAdd(LPOBJ lpObj,char* arg) // OK
{
	...
	if(strcmp(arg,"") == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,454);
		return;
	}
	...
	if(strlen(syntax) < 3)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,459);
		return;
	}

	if(lpObj->Pick.ItemCount >= 20)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,460);
		return;
	}
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,455,lpObj->Pick.ItemCount,syntax);
...
void CCustomPick::CommandCustomClear(LPOBJ lpObj) // OK
{
	...
	gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,456);
...
void CCustomPick::OnPickClose(LPOBJ lpObj) // OK
{
	if(lpObj->Pick.Started != 0)
	{
		...
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,453);
	}
}
