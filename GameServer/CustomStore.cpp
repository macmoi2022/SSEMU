...
void CCustomStore::SecondProc(LPOBJ lpObj) // OK
{
	if(lpObj->Store.Offline != 0)
	{
		if(lpObj->Store.Offline == 2)
		{
			if((--lpObj->Store.OfflineTime) == 0)
			{
				this->StoreOfflineClose(lpObj);
			}
		}
...
void CCustomStore::StoreOfflineClose(LPOBJ lpObj) // OK
{
	if(lpObj->Store.Offline != 0)
	{
...
void CCustomStore::ReadCustomStoreInfo(char* section,char* path) // OK
{
	...
	this->m_CustomStoreSkinChange = GetPrivateProfileInt(section, "CustomStoreSkinChange", 0, path);
...
bool CCustomStore::CommandCustomStoreOffline(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Store.Open == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,540);
		return 0;
	}

	if(this->m_CustomStoreOfflineMapZone != -1 && this->m_CustomStoreMapList[1][lpObj->Map] == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,574);
		return 0;
	}

	if(gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,MAP_ATTR_SAFE) == 0)
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,541);
		return 0;
	}
	...
	lpObj->Store.Offline = 1;

	lpObj->Store.OfflineTime = 0;

	if (gCustomStore->m_CustomStoreSkinChange != 0)
	{
		lpObj->Change = gCustomStore->m_CustomStoreSkinChange;
	}
...
bool CCustomStore::OpenCustomStore(LPOBJ lpObj,int type) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage->GetText(806, gServerInfo->m_ServerLang));
		return 0;
	}
	...
	{
		gNotice->GCNoticeSend(lpObj->Index,1,0,0,0,0,0,542);
		return 0;
	}
	...
	lpObj->Store.Custom = 1;

	lpObj->Store.CustomType = type;
	...
	{
		case 0:
			memcpy(lpObj->Store.Text,gMessage->GetText(543,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 1:
			memcpy(lpObj->Store.Text,gMessage->GetText(544,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 2:
			memcpy(lpObj->Store.Text,gMessage->GetText(545,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 3:
			memcpy(lpObj->Store.Text,gMessage->GetText(546,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 4:
			memcpy(lpObj->Store.Text,gMessage->GetText(547,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 5:
			memcpy(lpObj->Store.Text,gMessage->GetText(548,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 6:
			memcpy(lpObj->Store.Text,gMessage->GetText(549,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 7:
			memcpy(lpObj->Store.Text,gMessage->GetText(550,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 8:
			memcpy(lpObj->Store.Text,gMessage->GetText(551,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
		case 9:
			memcpy(lpObj->Store.Text,gMessage->GetText(552,gServerInfo->m_ServerLang),sizeof(lpObj->Store.Text));
			break;
	}
	...
	memcpy(pMsg.text,lpObj->Store.Text,sizeof(pMsg.text));
...
{
	if(lpObj->Store.Custom != 0)
	{
		lpObj->Store.Custom = 0;
		lpObj->Store.CustomType = 0;
	}

	if(lpObj->Store.Offline == 1)
	{
		lpObj->Store.Offline = 2;
		lpObj->Store.OfflineTime = 5;
	}
}

void CCustomStore::OnPShopItemList(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(lpTarget->Store.Custom != 0)
	{
		switch(lpTarget->Store.CustomType)
		{
			case 0:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,553);
				break;
			case 1:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,554);
				break;
			case 2:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,555);
				break;
			case 3:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,556);
				break;
			case 4:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,557);
				break;
			case 5:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,558);
				break;
			case 6:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,559);
				break;
			case 7:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,560);
				break;
			case 8:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,561);
				break;
			case 9:
				gNotice->GCNoticeSend(lpObj->Index,0,0,0,0,0,0,562);
				break;
		}
	}
}
...
bool CCustomStore::OnPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpTarget->Store.Transaction != 0)
	{
	...
	if(lpTarget->Store.CustomType == 7 || lpTarget->Store.CustomType == 8 || lpTarget->Store.CustomType == 9)
	{
		#if(GAMESERVER_SHOP==1)
	...
	int PShopJoBValue = ((lpTarget->Store.CustomType==0)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);

	int PShopJoSValue = ((lpTarget->Store.CustomType==1)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);

	int PShopJoLValue = ((lpTarget->Store.CustomType==2)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);

	int PShopJoCValue = ((lpTarget->Store.CustomType==3)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);

	int PShopJCRValue = ((lpTarget->Store.CustomType==4)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);
	
	int PShopJoGValue = ((lpTarget->Store.CustomType==5)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);
	
	int PShopJoHValue = ((lpTarget->Store.CustomType==6)?lpTarget->Inventory[lpMsg->slot].m_PShopValue:0);
	...
	{
		lpTarget->Store.ItemChange = 1;
	}
	else
	{
		lpTarget->Store.Open = 0;
		memset(lpTarget->Store.Text,0,sizeof(lpTarget->Store.Text));
		gPersonalShop->GCPShopCloseSend(bIndex,1);
		this->OnPShopClose(lpTarget);
	}

	lpTarget->Store.Transaction = 0;
...
void CCustomStore::OnPShopBuyItemCallbackRecv(LPOBJ lpObj,LPOBJ lpTarget,DWORD slot,DWORD Coin1,DWORD Coin2,DWORD Coin3) // OK
{
	...
	DWORD PShopCoin1Value = ((lpTarget->Store.CustomType==7)?lpTarget->Inventory[slot].m_PShopValue:0);

	DWORD PShopCoin2Value = ((lpTarget->Store.CustomType==8)?lpTarget->Inventory[slot].m_PShopValue:0);

	DWORD PShopCoin3Value = ((lpTarget->Store.CustomType==9)?lpTarget->Inventory[slot].m_PShopValue:0);
