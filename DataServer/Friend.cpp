...
void CFriend::GDFriendListRecv(SDHP_FRIEND_LIST_RECV* lpMsg,int index) // OK
{
	...
	if(gQueryManager->ExecQuery("SELECT FriendName FROM T_FriendList WHERE GUID=%d",guid) != 0)
	{
		...
		while(gQueryManager->Fetch() != SQL_NO_DATA)
		{
			...
			FriendList.server = (BYTE)GetServerCodeByName(FriendList.Name);
...
void CFriend::DGFriendStateSend(char* name,int state)
{
	...
	if(gQueryManager->ExecQuery("SELECT FriendName FROM T_FriendList WHERE GUID=%d",guid,0) != 0)
	{
		while(gQueryManager->Fetch() != SQL_NO_DATA)
		{
			SDHP_FRIEND_STATE_SEND pMsg;
			...
			if(gCharacterManager->GetCharacterInfo(&CharacterInfo,pMsg.Name) != 0)
			{
				...
				pMsg.server = ((state==0)?GetServerCodeByName(pMsg.Target):0xFF);
